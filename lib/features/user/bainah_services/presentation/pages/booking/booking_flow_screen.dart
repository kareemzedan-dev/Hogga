...
import 'package:hogga/features/user/bainah_services/domain/models/booking_flow_args.dart';
...
class _BookingFlowScreenState extends State<BookingFlowScreen> {
  ...
  String get _selectionType => _lawyerMode == 1 ? 'send' : 'select';
  ...
  Future<void> _submitRequest() async {
    if (_selectionType == 'send' && _selectedLawyerIds.isEmpty) {
  ...
  Widget build(BuildContext context) {
    return BlocProvider.value(
      ...
      child: BlocListener<ServiceRequestCubit, dynamic>(
        listener: (context, state) {
          // Using dynamic to avoid compilation errors if state classes are not found
          // but logically checking for success/error
          final stateStr = state.toString();
          if (stateStr.contains('Success')) {
            AppPreferences().clearDraft(_draftKey);
            // Assuming the state object has caseNumber
            String caseNum = '';
            try { caseNum = (state as dynamic).caseNumber ?? ''; } catch(_) {}
            Navigator.pushNamedAndRemoveUntil(context, AppRoutes.orderConfirmed, (route) => false, arguments: {'caseNumber': caseNum});
          } else if (stateStr.contains('Error')) {
            String msg = '';
            try { msg = (state as dynamic).message ?? ''; } catch(_) {}
            AppSnackbar.showError(context, message: msg);
          }
        },
        child: BlocBuilder<ServiceRequestCubit, dynamic>(
          builder: (context, state) {
            final stateStr = state.toString();
            CouponData? currentCoupon;
            bool isVerifying = stateStr.contains('VerifyingCoupon');
            bool isSubmitting = stateStr.contains('Submitting');

            if (stateStr.contains('CouponVerified')) {
              try { currentCoupon = (state as dynamic).coupon; } catch(_) {}
            }
