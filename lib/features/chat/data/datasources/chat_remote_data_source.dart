...
class ChatRemoteDataSource {
  ...
  Future<List<ChatRoomModel>> getChatRooms() async {
    final response = await apiClient.get(AppEndPoints.chats);
  ...
  Future<ChatMessagesResponse> getMessages(int roomId, {int page = 1}) async {
    final response = await apiClient.get(
      AppEndPoints.chatMessages(roomId),
  ...
  Future<ChatMessageModel> sendMessage(
    ...
    final response = await apiClient.post(
      AppEndPoints.sendChatMessage(roomId),
      data: formData,
    );
  ...
  Future<CallTokenModel> getCallToken(int roomId) async {
    final response = await apiClient.get(AppEndPoints.callToken(roomId));
  ...
  Future<void> updateCallStatus(int callId, String status) async {
    await apiClient.post(
      AppEndPoints.callStatus(callId),
      data: {'status': status},
    );
  }
}
