import 'package:flutter/material.dart';

import '../utils/app_strings.dart';

class AppLocalizations {
  final Locale locale;

  AppLocalizations(this.locale);

  static AppLocalizations? of(BuildContext context) {
    return Localizations.of<AppLocalizations>(context, AppLocalizations);
  }

  static const LocalizationsDelegate<AppLocalizations> delegate =
      _AppLocalizationsDelegate();

  static final Map<String, Map<String, String>> _localizedValues = {
    'en': {
      AppStrings.appName: 'Hogga',
      AppStrings.noRouteFound: 'No Route Found',
      AppStrings.login: 'Login',
      AppStrings.register: 'Register',
      AppStrings.email: 'Email',
      AppStrings.password: 'Password',
      AppStrings.confirmPassword: 'Confirm Password',
      AppStrings.forgotPassword: 'Forgot Password?',
      AppStrings.sendResetLink: 'Send Reset Link',
      AppStrings.resetEmailSent: 'Reset Link Sent',
      AppStrings.profile: 'Profile',
      AppStrings.requiredField: 'Required field',
      AppStrings.passwordMismatch: 'Passwords do not match',
      AppStrings.invalidPhone: 'Invalid phone number',
      AppStrings.invalidEmail: 'Invalid email address',
      AppStrings.passwordTooShort: 'Minimum 8 characters required',
      AppStrings.invalidCivilID: 'Invalid Civil ID (8 digits)',
      AppStrings.invalidLawyerLicense: 'Invalid license number',
      AppStrings.registerAsUser: 'Create User Account',
      AppStrings.language: 'Language',
      AppStrings.aboutUs: 'About Us',
      AppStrings.aboutApp: 'About App',
      AppStrings.contactUs: 'Contact Us',
      AppStrings.privacyPolicy: 'Privacy Policy',
      AppStrings.logout: 'Logout',
      AppStrings.instructions: 'Usage Guidelines',
      AppStrings.lawyerWallet: 'Wallet',
      AppStrings.freeConsultations: 'Free Consultations',
      AppStrings.yourConsultationPercentage: 'Your percentage: {percentage}%',
      AppStrings.used: 'Used',
      AppStrings.remaining: 'Remaining',
      AppStrings.campaignNotActive: 'Campaign is not active currently',
      AppStrings.rewardValue: 'Reward Value',
      AppStrings.referralCodeTitle: 'Referral Code',
      AppStrings.codeCopied: 'Code copied successfully',
      AppStrings.bookings: 'Bookings',

      AppStrings.termsAgreementIntro: 'By creating an account, you agree to',
      AppStrings.and: 'and',
      AppStrings.termsOfUseTitle: 'Usage Guidelines',
      AppStrings.welcomeTitle: 'Start Your Journey with Hogga',
      AppStrings.welcomeSubtitle: 'Choose account type to continue',
      AppStrings.userCardSubtitle: 'Trusted legal & family consultations',
      AppStrings.lawyerCardSubtitle: 'Provide legal & family consultations',
      AppStrings.registerAsLawyer: 'Create Consultant Account',
      AppStrings.myOrders: 'Cases',
      AppStrings.noOrdersYet: 'No cases yet',
      AppStrings.loginSubtitle: 'Welcome back',
      AppStrings.dontHaveAccount: 'Don\'t have an account? ',
      AppStrings.alreadyHaveAccount: 'Already have an account? ',
      AppStrings.male: 'Male',
      AppStrings.female: 'Female',
      AppStrings.agreeToTermsError: 'Please agree to the usage guidelines',
      AppStrings.individual: 'Individual',
      AppStrings.corporate: 'Corporate',
      AppStrings.institution: 'Institution',
      AppStrings.iAgreeTo: 'I agree to ',
      AppStrings.chooseCountry: 'Choose Country',
      AppStrings.accountTier: 'Account Type',
      AppStrings.fullNameHint: 'Full Name',
      AppStrings.phoneStepTitle: 'Mobile Number',
      AppStrings.nameStepTitle: 'Register Name',
      AppStrings.passwordStepTitle: 'Create Password',
      AppStrings.phoneStepSubtitle:
          'Enter your mobile number to create a new account',
      AppStrings.nameStepSubtitle: 'Write your full name',
      AppStrings.passwordStepSubtitle: 'Choose a secure password',
      AppStrings.createAccountButton: 'Create Account',
      AppStrings.activationCode: 'Activation Code',
      AppStrings.didNotReceiveCode: 'Didn\'t receive the code?',
      AppStrings.resendAfter: 'Resend after',
      AppStrings.seconds: 's',
      AppStrings.resendNow: 'Resend now',
      AppStrings.whatsYourPhone: 'What is your phone number?',
      AppStrings.areYouLawyer: 'Are you a lawyer? ',
      AppStrings.emailTaken: 'The email has already been taken',
      AppStrings.phoneTaken: 'The phone number is already registered',
      AppStrings.invalidCredentials: 'Email or password is incorrect',
      AppStrings.serverError: 'Something went wrong, please try again later',
      AppStrings.sessionExpired: 'Your session has expired, please login again',
      AppStrings.sevenMinutesFree: 'You have 7 minutes of free consultation',
      AppStrings.availabilityToday: 'Available today from 10:00 to 23:00',
      AppStrings.howCanWeHelp: 'How can we help you?',
      AppStrings.legalConsultationDesc:
          'Get legal advice from a licensed lawyer to help you understand your legal position and make the right decision.',
      AppStrings.judiciaryExecutionDesc:
          'Specialized services in judiciary and execution include writing legal briefs and lists, pleading and representation before courts, case studies, attending sessions, and following up on execution requests.',
      AppStrings.businessServicesDesc:
          'Legal services to support the business sector include drafting and reviewing contracts and agreements, in addition to registering and protecting trademarks.',
      AppStrings.otherServicesDesc:
          'Various legal and administrative services include reviewing government departments and agencies, and providing other customized services according to the client\'s needs.',
      AppStrings.bannerTitle1: 'Your legal consultation starts',
      AppStrings.bannerPrice1: 'From 100 OMR only!',
      AppStrings.bookNow: 'Book Now',
      AppStrings.bannerTitle2: 'Immediate legal consultation',
      AppStrings.bannerPrice2: 'With a specialized lawyer',
      AppStrings.startNow: 'Start Now',
      AppStrings.bannerTitle3: 'Integrated legal services',
      AppStrings.bannerPrice3: 'For individuals and companies',
      AppStrings.learnMore: 'Learn More',
      AppStrings.bannerDesc:
          'Talk to a specialized lawyer now through a live call',
      AppStrings.startCallNow: 'Start call now',
      AppStrings.darkMode: 'Dark Mode',
      AppStrings.lightMode: 'Light Mode',
      AppStrings.home: 'Home',
      AppStrings.personalInfo: 'Personal Information',
      AppStrings.securitySettings: 'Security Settings',
      AppStrings.editProfile: 'Edit Profile',
      AppStrings.currentPassword: 'Current Password',
      AppStrings.newPassword: 'New Password',
      AppStrings.confirmNewPassword: 'Confirm New Password',
      AppStrings.saveChanges: 'Save Changes',
      AppStrings.changePhoto: 'Change Photo',
      AppStrings.skip: 'Skip',
      AppStrings.onboardingTitle1:
          'Receive offers from the best\nlicensed lawyers',
      AppStrings.onboardingSubtitle1:
          'High-quality legal services\nand consultations',
      AppStrings.onboardingTitle2:
          'Your legal consultations\nat any time and place',
      AppStrings.onboardingSubtitle2:
          'Connect with a specialist lawyer\nvia call or online meeting',
      AppStrings.onboardingTitle3: 'Follow your case\nstep by step',
      AppStrings.onboardingSubtitle3:
          'Upload documents and follow your\ncase stages completely and transparently',
      AppStrings.onboardingLawyer1: 'Sultan Al-Balushi',
      AppStrings.onboardingLawyer2: 'Ahmed Al-Rawahi',
      AppStrings.onboardingSpec1: 'Commercial Law',
      AppStrings.onboardingSpec2: 'Family Law',
      AppStrings.onboardingStatusAvailable: 'Available',
      AppStrings.onboardingStatusRunning: 'Running',
      AppStrings.onboardingPrice1: '45 OMR',
      AppStrings.onboardingPrice2: '30 OMR',
      AppStrings.onboardingDetails: 'Details',
      AppStrings.onboardingCall: 'Call',
      AppStrings.onboardingInPerson: 'In Person',
      AppStrings.onboardingOnline: 'Online',
      AppStrings.onboardingCaseNo: 'Case No #3312',
      AppStrings.onboardingStepNew: 'New Request',
      AppStrings.onboardingStepReview: 'Under Review',
      AppStrings.onboardingStepRunning: 'In Progress',
      AppStrings.onboardingStepCompleted: 'Completed',
      AppStrings.ongoingConsultations: 'Current',
      AppStrings.consultationHistory: 'Previous',
      AppStrings.noOrdersInSection: 'No records in this section',
      AppStrings.hoggaServicesHub: 'Hogga Services',
      AppStrings.chooseSpecialization: 'Choose Specialization',
      AppStrings.legalConsultationsTab: 'Legal Consultations',
      AppStrings.judiciaryExecutionTab: 'Judiciary & Execution',
      AppStrings.businessServicesTab: 'Business Services',
      AppStrings.otherServicesTab: 'Other Services',
      AppStrings.immediateConsultation: 'Immediate Consultation',
      AppStrings.immediateConsultationDesc:
          'Instantly connect with a lawyer via call. No prior booking needed.',
      AppStrings.writtenConsultation: 'Written Consultation',
      AppStrings.writtenConsultationDesc:
          'Communicate in writing with a lawyer to ask your questions and send documents via chat.',
      AppStrings.scheduledConsultation: 'Scheduled Consultation',
      AppStrings.scheduledConsultationDesc:
          'Book a consultation with a lawyer of your choice at your preferred time via call.',
      AppStrings.legalWritings: 'Legal Writings',
      AppStrings.legalWritingsDesc:
          'Drafting regulations, response memos, lawsuits, appeals, and other legal documents.',
      AppStrings.pleadingAndAttorneyship: 'Pleading & Attorneyship',
      AppStrings.pleadingAndAttorneyshipDesc:
          'Appoint a lawyer to represent you in court until a judgment is issued.',
      AppStrings.sessionAttendance: 'Session Attendance',
      AppStrings.sessionAttendanceDesc:
          'If you are busy, a lawyer can attend the session on your behalf.',
      AppStrings.caseStudy: 'Case Study',
      AppStrings.caseStudyDesc:
          'Detailed study of your case, contracts, and documents for accurate answers.',
      AppStrings.executionRequests: 'Execution Requests',
      AppStrings.executionRequestsDesc:
          'Filing execution requests and following procedures before the execution court.',
      AppStrings.govAgenciesReview: 'Gov Agencies Review',
      AppStrings.govAgenciesReviewDesc:
          'A lawyer reviews ministries, prosecution, police stations, or any government entity for you.',
      AppStrings.otherLegalServices: 'Other Legal Services',
      AppStrings.otherLegalServicesDesc:
          'Any other legal services you wish to obtain from a lawyer.',
      AppStrings.specPersonalStatus: 'Personal Status',
      AppStrings.specPersonalStatusDesc:
          'Marriage, divorce, alimony, custody, and visitation rights.',
      AppStrings.specInheritance: 'Inheritance & Estates',
      AppStrings.specInheritanceDesc:
          'Division of inheritance, estates, and execution of wills.',
      AppStrings.specCriminal: 'Criminal',
      AppStrings.specCriminalDesc:
          'Theft, fraud, blackmail, and all criminal offenses.',
      AppStrings.specPublicRights: 'Public Rights',
      AppStrings.specPublicRightsDesc:
          'Financial claims, real estate disputes, and general legal matters.',
      AppStrings.specLabor: 'Labor',
      AppStrings.specLaborDesc:
          'Disputes between employees and employers, wages, and termination.',
      AppStrings.specCommercial: 'Commercial',
      AppStrings.specCommercialDesc:
          'Commercial transactions, partner disputes, bankruptcy, and liquidation.',
      AppStrings.specIntellectualProperty: 'Intellectual Property',
      AppStrings.specIntellectualPropertyDesc:
          'Patents, copyrights, trademarks, and trade secrets.',
      AppStrings.specAdministrative: 'Administrative',
      AppStrings.specAdministrativeDesc:
          'Cancellation of administrative decisions and claims against government entities.',
      AppStrings.specExecution: 'Execution Cases',
      AppStrings.specExecutionDesc:
          'Execution of judgments, settlement reports, checks, and lease contracts.',
      AppStrings.specQuasiJudicial: 'Quasi-Judicial Committees',
      AppStrings.specQuasiJudicialDesc:
          'Traffic violations, customs, taxes, and banking disputes.',
      AppStrings.nextStep: 'Next',
      AppStrings.chooseServiceType: 'Choose Service Type',
      AppStrings.subtypeLawsuit: 'Lawsuit',
      AppStrings.subtypeLawsuitDesc:
          'Formal document submitted to the court to obtain legal rights.',
      AppStrings.subtypeReplyMemo: 'Response Memo',
      AppStrings.subtypeReplyMemoDesc:
          'Legal document responding to a lawsuit filed against you.',
      AppStrings.subtypeReplyJoinder: 'Joinder Reply',
      AppStrings.subtypeReplyJoinderDesc:
          'Document for exchanging responses and defenses between parties.',
      AppStrings.subtypeObjection: 'Objection List',
      AppStrings.subtypeObjectionDesc:
          'Document responding to the lawsuit after the first session.',
      AppStrings.subtypeCassation: 'Cassation List',
      AppStrings.subtypeCassationDesc:
          'Submitted to the Supreme Court to appeal a judgment.',
      AppStrings.stepSendRequest: 'Send Request',
      AppStrings.stepChooseLawyer: 'Choose Lawyer',
      AppStrings.stepPayment: 'Payment',
      AppStrings.requestTitle: 'Request Title',
      AppStrings.requestTitleHint: 'Please write a clear and concise title',
      AppStrings.requestDetails: 'Request Details',
      AppStrings.requestDetailsHint:
          'Provide as much detail as possible to help the lawyer understand your case...',
      AppStrings.remoteAttendance: 'Can attend remotely?',
      AppStrings.governorate: 'Governorate Name',
      AppStrings.governorateHint: 'Enter governorate name',
      AppStrings.city: 'City Name',
      AppStrings.cityHint: 'Enter city name',
      AppStrings.infoDialogStepLabel: 'Details & Requirements',
      AppStrings.infoDialogHeading: 'Enter basic details',
      AppStrings.infoDialogBody:
          'Write a general summary without sensitive data. Share details later with the lawyer.',
      AppStrings.infoDialogConfirm: 'I understand',
      AppStrings.howToSendRequest: 'How do you want to send your request?',
      AppStrings.serviceFee: 'Service Fee',
      AppStrings.serviceFeeDesc:
          'Get price offers from +1000 licensed lawyers and start negotiating easily.',
      AppStrings.broadcastToAll: 'Send to all lawyers immediately',
      AppStrings.broadcastToAllDesc:
          'Your request will reach over 584 lawyers ready to respond.',
      AppStrings.chooseLawyersManually: 'Choose lawyers manually',
      AppStrings.chooseLawyersManuallyDesc:
          'Select your preferred lawyers to send directly to them.',
      AppStrings.searchLawyer: 'Search for a lawyer',
      AppStrings.sortBy: 'Sort by',
      AppStrings.lawyerType: 'Lawyer Type',
      AppStrings.availableLawyers: 'Available Lawyers',
      AppStrings.selectOneLawyerOnly: 'Select one lawyer only',
      AppStrings.selectLawyerToShowPrices:
          'Select a lawyer first to show available prices',
      AppStrings.verifiedLawyer: 'Verified',
      AppStrings.addAll: 'Add All',
      AppStrings.addLawyer: '+ Add Lawyer',
      AppStrings.availableNow: 'Available Now',
      AppStrings.yearsExperience: 'Experience',
      AppStrings.requestsCount: 'Requests',
      AppStrings.chooseCity: 'Choose City',
      AppStrings.searchCity: 'Search City',
      AppStrings.havePromoCode: 'Have a promo code?',
      AppStrings.enterPromoCode: 'Enter code here',
      AppStrings.applyCode: 'Apply',
      AppStrings.payWithCard: 'Pay with Card',
      AppStrings.payWithWalet: 'Pay with Wallets',
      AppStrings.serviceValue: 'Service Value:',
      AppStrings.taxValue: 'Tax Value:',
      AppStrings.totalDue: 'Total Due:',
      AppStrings.paymentDisclaimer:
          'To protect your rights, ensure payment is only done within the platform.',
      AppStrings.totalAmount: 'Total Amount Due',
      AppStrings.completeOrder: 'Complete Order',
      AppStrings.confirmCompleteOrder:
          'Are you sure you want to mark this order as completed?',
      AppStrings.completeService: 'Confirm Service Completion',
      AppStrings.confirmCompleteService:
          'Are you sure this service has been completed?',
      AppStrings.completeConsultation: 'Complete Consultation',
      AppStrings.confirmCompleteConsultation:
          'Are you sure this consultation has been completed?',
      AppStrings.currencySymbol: 'OMR',
      AppStrings.orderSentSuccessfully: 'Order Sent Successfully',
      AppStrings.orderProcessingDesc:
          'Your order is processing, you will receive offers from lawyers soon.',
      AppStrings.orderNumber: 'Order Number',
      AppStrings.trackOrder: 'Track Order',
      AppStrings.backToHome: 'Back to Home',
      AppStrings.startCallSubtitle:
          'Direct voice/video consultation with the lawyer',
      AppStrings.enterChatRoom: 'Enter Chat Room',
      AppStrings.enterChatSubtitle: 'Start chatting directly with the lawyer',
      AppStrings.viewServiceDetails: 'View Service Details',
      AppStrings.viewServiceDetailsSubtitle:
          'Follow up on case details and documents',
      AppStrings.consultationConfirmedSuccessfully:
          'Consultation Confirmed Successfully!',
      AppStrings.consultationReadyDesc:
          'Your consultation is confirmed and the lawyer is ready to connect with you.',
      AppStrings.quickActions: 'Quick Actions',
      AppStrings.loadingCommunicationDetails:
          'Preparing communication details...',
      AppStrings.years: 'Years',
      AppStrings.addedSuccessfully: 'Added',
      AppStrings.all: 'All',
      AppStrings.maleLawyer: 'Lawyer (Male)',
      AppStrings.femaleLawyer: 'Lawyer (Female)',
      AppStrings.defaultSort: 'Default',
      AppStrings.byRating: 'By Rating',
      AppStrings.byExperience: 'By Experience',
      AppStrings.highestRated: 'Highest Rated',
      AppStrings.mostExperienced: 'Most Experienced',
      AppStrings.confirmLawyersSelection: 'Confirm {count} Lawyers',
      AppStrings.consultationDetails: 'Details',
      AppStrings.wallet: 'Payments',
      AppStrings.deleteAccount: 'Delete Account',
      AppStrings.deleteAccountTitle: 'Delete Account',
      AppStrings.deleteAccountSubtitle:
          'Are you sure you want to delete your account? This action cannot be undone.',
      AppStrings.deleteAccountAction: 'Delete My Account',
      AppStrings.totalPayments: 'Total Payments',
      AppStrings.accountActive: 'Account Active',
      AppStrings.completed: 'Completed',
      AppStrings.phone: 'Phone',
      AppStrings.emailOrPasswordNotValid: 'Email or password is incorrect',
      AppStrings.cancelOrder: 'Cancel',
      AppStrings.cancelCase: 'Cancel',
      AppStrings.confirmCancelCase:
          'Are you sure you want to cancel this case?',
      AppStrings.contactLegalOffice: 'Contact Legal Office',
      AppStrings.referenceNumber: 'Reference Number',
      AppStrings.consultationSubject: 'Consultation Subject',
      AppStrings.consultationPlaceholder:
          'General legal consultation on regulations.',
      AppStrings.attachedDocuments: 'Attached',
      AppStrings.reviewStep: 'Review',
      AppStrings.analysisStep: 'Analysis',
      AppStrings.appointmentStep: 'Appointment',
      AppStrings.executionStep: 'Execution',
      AppStrings.rateConsultation: 'Rate the Consultation',
      AppStrings.rateConsultationDesc:
          'Your opinion helps improve our legal services.',
      AppStrings.additionalNotes: 'Additional Notes:',
      AppStrings.writeNotesHint: 'Write your notes here (optional)...',
      AppStrings.financialStatement: 'Financial Statement',
      AppStrings.consultationFee: 'Consultation Fee',
      AppStrings.subtotal: 'Subtotal',
      AppStrings.specialDiscount: 'Special Discount',
      AppStrings.totalProfessionalFees: 'Total Professional Fees:',
      AppStrings.licensedLawyer: 'Licensed Lawyer',
      AppStrings.ratingLabel: 'Rating',
      AppStrings.aboutLawyer: 'About Lawyer',
      AppStrings.servicesAndPrices: 'Services & Prices',
      AppStrings.clientReviews: 'Client Reviews',
      AppStrings.viewAll: 'View All',
      AppStrings.requestConsultationNow: 'Request Consultation Now',
      AppStrings.receptionSettings: 'Reception',
      AppStrings.acceptChatConsultations: 'Text Consults',
      AppStrings.acceptInstantConsultations: 'Instant Consults',
      AppStrings.acceptServices: 'Legal Services',
      AppStrings.performanceOverview: 'Performance Overview',
      AppStrings.activeCasesLabel: 'Active Cases',
      AppStrings.monthlyIncome: 'Monthly Income',
      AppStrings.videoCall: 'Call',
      AppStrings.muscatRegion: 'Muscat Governorate',
      AppStrings.batinahNorthRegion: 'North Al Batinah Governorate',
      AppStrings.dhofarRegion: 'Dhofar Governorate',
      AppStrings.dakhiliyahRegion: 'Ad Dakhiliyah Governorate',
      AppStrings.comma: ',',
      AppStrings.legalLibrary: 'Legal Library',
      AppStrings.myTasks: 'My Tasks',
      AppStrings.reports: 'Reports',
      AppStrings.clientsRating: 'Clients Rating',
      AppStrings.allAppointments: 'All Appointments',
      AppStrings.availableOnline: 'Available',
      AppStrings.busy: 'Busy',
      AppStrings.availableBalance: 'Available Balance',
      AppStrings.lastTransactions: 'Last Transactions',
      AppStrings.withdrawalActionMsg:
          'Withdrawal requests for the available balance will be activated soon.',
      AppStrings.pendingEarnings: 'Pending Earnings',
      AppStrings.deductedCommission: 'Deducted Commission',
      AppStrings.viewOptions: 'View Options',
      AppStrings.byDateNewest: 'By Date (Newest First)',
      AppStrings.byAmountHighest: 'By Amount (Highest First)',
      AppStrings.onlyBankTransfers: 'Bank Transfers Only',
      AppStrings.noTransactions: 'No Transactions',
      AppStrings.noTransactionsSubtitle:
          'No transactions found in this category.',
      AppStrings.income: 'Income',
      AppStrings.expenses: 'Expenses',
      AppStrings.myDailyTasks: 'My Daily Tasks',
      AppStrings.upcomingTasks: 'Upcoming Tasks',
      AppStrings.totalTasks: 'Total Tasks',
      AppStrings.pendingAction: 'Pending Action',
      AppStrings.highPriority: 'High',
      AppStrings.mediumPriority: 'Medium',
      AppStrings.lowPriority: 'Low',
      AppStrings.today: 'Today',
      AppStrings.tomorrow: 'Tomorrow',
      AppStrings.am: 'AM',
      AppStrings.pm: 'PM',
      AppStrings.sunday: 'Sunday',
      AppStrings.monday: 'Monday',
      AppStrings.tuesday: 'Tuesday',
      AppStrings.wednesday: 'Wednesday',
      AppStrings.thursday: 'Thursday',
      AppStrings.friday: 'Friday',
      AppStrings.saturday: 'Saturday',
      AppStrings.appNameArabic: 'Hogga',
      AppStrings.splashSubtitle: 'Your legal consultations at your fingertips',
      AppStrings.joinhoggaTeam: 'Join Hogga Team',
      AppStrings.stepLabel: 'Step',
      AppStrings.stepDataLabel: 'Data',
      AppStrings.stepAccount: 'Account',
      AppStrings.stepIdentity: 'Identity',
      AppStrings.stepSpecialization: 'Specialization',
      AppStrings.stepServices: 'Services',
      AppStrings.stepDocuments: 'Documents',
      AppStrings.stepReview: 'Review',
      AppStrings.stepStatus: 'Status',
      AppStrings.confirmAndSendRequest: 'Confirm and Send Request',
      AppStrings.continueAction: 'Continue',
      AppStrings.createAccountTitle: 'Create Your Account',
      AppStrings.createAccountSubtitle:
          'Enter your basic details to start with hogga',
      AppStrings.otpSentSubtitle:
          'Enter the verification code sent to your phone',
      AppStrings.otpSentNotice:
          'Verification code has been sent to the registered phone',
      AppStrings.resendCodeAction: 'Resend Code?',
      AppStrings.personalDataTitle: 'Personal Data',
      AppStrings.personalDataSubtitle:
          'This data helps us verify your personal identity',
      AppStrings.fullNameIdHint: 'Full Name (as in ID)',
      AppStrings.civilIdHint: 'Civil ID / National ID',
      AppStrings.requiredError: 'Required',
      AppStrings.fullNameMinError: 'Please enter at least three names',
      AppStrings.professionalDataTitle: 'Professional Data',
      AppStrings.professionalDataSubtitle:
          'Enter your professional legal practice details',
      AppStrings.licenseNumberHint: 'Professional License Number',
      AppStrings.degreeHint: 'Degree of Enrollment',
      AppStrings.experienceYearsHint: 'Years of Experience',
      AppStrings.legalSpecsTitle: 'Legal Specializations',
      AppStrings.servicesAndPricesTitle: 'Services and Prices',
      AppStrings.servicesAndPricesSubtitle:
          'Specify the services you provide and the price for each service in Omani Rial (O.R)',
      AppStrings.setPricesTitle: 'Setting prices for each service',
      AppStrings.minOneServiceError: 'Please select at least one service',
      AppStrings.priceHint: 'Price',
      AppStrings.fixedType: 'Fixed',
      AppStrings.averageType: 'Average',
      AppStrings.officialDocsTitle: 'Official Documents',
      AppStrings.officialDocsSubtitle:
          'Please upload clear photos of the required documents to verify professionalism',
      AppStrings.idCardPhoto: 'ID Card Photo',
      AppStrings.licensePhoto: 'Syndicate Card / License Photo',
      AppStrings.registrationCertOptional:
          'Registration Certificate (Optional)',
      AppStrings.clickToUpload: 'Click to upload',
      AppStrings.reviewApplicationTitle: 'Review Your Application',
      AppStrings.reviewApplicationSubtitle:
          'Ensure all entered data is correct before sending the request for review',
      AppStrings.experienceLabel: 'Experience',
      AppStrings.specializationsLabel: 'Specializations',
      AppStrings.applicationUnderReviewTitle: 'Your Request is Under Review',
      AppStrings.applicationUnderReviewSubtitle:
          'We are now working on verifying your data and legal documents to ensure the quality of the hogga team. We will notify you via text message upon acceptance.',
      AppStrings.legalConsultation: 'Legal Consultation',
      AppStrings.prefsAndApp: 'Preferences and App',
      AppStrings.clearAll: 'Clear All',
      AppStrings.newLegalRequest: 'New Legal Request',
      AppStrings.sessionReminder: 'Session Reminder',
      AppStrings.caseUpdate: 'Case Update',
      AppStrings.newDocumentUploaded: 'New Document Uploaded',
      AppStrings.consultationBooking: 'Consultation Booking',
      AppStrings.since: 'Since',
      AppStrings.minutesLabel: 'Minutes',
      AppStrings.callMinutes: 'Remaining Call Minutes',
      AppStrings.callEnded: 'Call Ended',
      AppStrings.reconnectingCall: 'Reconnecting...',
      AppStrings.callDurationLabel: 'Call Duration',
      AppStrings.ok: 'OK',
      AppStrings.performanceReports: 'Performance Reports',
      AppStrings.weekly: 'Weekly',
      AppStrings.monthly: 'Monthly',
      AppStrings.yearly: 'Yearly',
      AppStrings.annualIncome: 'Annual Income',
      AppStrings.consultationIncome: 'Consultation Income',
      AppStrings.recentWeeks: 'Recent Weeks',
      AppStrings.recentMonths: 'Recent Months',
      AppStrings.activitySummary: 'Activity Summary',
      AppStrings.completedCases: 'Completed Cases',
      AppStrings.clientSatisfaction: 'Client Satisfaction',
      AppStrings.basedOnEvaluations: 'Based on {count} evaluations',
      AppStrings.noClients: 'No Clients',
      AppStrings.noClientsSubtitle: 'No clients have contacted you yet.',
      AppStrings.searchByClientName: 'Search by client name...',
      AppStrings.activeCasesCount: '{count} Active Cases',
      AppStrings.callUnderDevelopment: 'Calling feature is under development',
      AppStrings.chatUnderDevelopment: 'Chat feature is under development',
      AppStrings.inProgress: 'Ongoing',
      AppStrings.noCases: 'No Cases',
      AppStrings.noCasesSubtitle: 'No cases have been assigned to you yet.',
      AppStrings.noSearchResultsSubtitle:
          'Try searching for another word or changing the filter.',
      AppStrings.searchByCaseIdOrTitle: 'Search by case ID or title...',
      AppStrings.caseNumber: 'Case Number: {id}',
      AppStrings.courtLabel: 'Court',
      AppStrings.primaryCourt: 'Primary',
      AppStrings.caseDetails: 'Case Details',
      AppStrings.upcoming: 'Upcoming',
      AppStrings.canceled: 'Canceled',
      AppStrings.scheduleTable: 'Schedule Table',
      AppStrings.noBookings: 'No Bookings',
      AppStrings.noBookingsSubtitle: 'You do not have any upcoming bookings.',
      AppStrings.availableForConsultations:
          'You are available for consultations',
      AppStrings.currentlyPaused: 'You are currently paused',
      AppStrings.profileVisibleNotice:
          'Your profile will be visible to clients',
      AppStrings.profileHiddenNotice:
          'Clients will not be able to book new appointments',
      AppStrings.confirmedStatus: 'Confirmed',
      AppStrings.pendingStatus: 'Pending',
      AppStrings.cancelledStatus: 'Cancelled',
      AppStrings.deliveredStatus: 'Delivered',
      AppStrings.shippedStatus: 'Shipped',
      AppStrings.unknownStatus: 'Unknown',
      AppStrings.cancelBooking: 'Cancel Booking',
      AppStrings.enterOtpSentTo: 'Enter the code sent to {email}',
      AppStrings.verificationSuccess: 'Verified successfully',
      AppStrings.resendCode: 'Resend Code',
      AppStrings.resendIn: 'Resend in {seconds}s',
      AppStrings.specializedConsultation: 'Specialized Consultation',
      AppStrings.contractDrafting: 'Contract Drafting',
      AppStrings.caseStudyService: 'Case Study',
      AppStrings.priceWithCurrency: '{price} OMR',
      AppStrings.comingSoon: 'Coming Soon',
      AppStrings.oman: 'Oman',
      AppStrings.saudi: 'Saudi Arabia',
      AppStrings.uae: 'United Arab Emirates',
      AppStrings.kuwait: 'Kuwait',
      AppStrings.qatar: 'Qatar',
      AppStrings.bahrain: 'Bahrain',
      AppStrings.egypt: 'Egypt',
      AppStrings.jordan: 'Jordan',
      AppStrings.redirectingToService: 'Redirecting to service request...',
      AppStrings.chatUnderDevelopmentSubtitle:
          'Chat feature is under development',
      AppStrings.notificationNewRequestDesc:
          'You have a new real estate consultation request from Khalid Al-Maamari.',
      AppStrings.notificationSessionReminderDesc:
          'Reminder: You have a session in the Primary Court tomorrow at 9:00 AM.',
      AppStrings.notificationCaseUpdateDesc:
          'Case status #4421 has been updated to "Awaiting Judgment".',
      AppStrings.notificationNewDocumentDesc:
          'The client uploaded "Revised Lawsuit List" in the compensation case.',
      AppStrings.notificationBookingDesc:
          'A new consultation appointment has been booked for next Tuesday.',
      AppStrings.omr: 'OMR',
      AppStrings.secondsUnit: 's',
      AppStrings.verifiedSuccessfully: 'Verified Successfully',
      AppStrings.sessionsHistory: 'Sessions History',
      AppStrings.orderDetail: 'orderDetail',
      AppStrings.accept: 'Accept',
      AppStrings.refuse: 'Refuse',
      AppStrings.acceptedSuccess: 'Consultation accepted',
      AppStrings.refusedSuccess: 'Consultation refused',
      AppStrings.waitingForApproval: 'Waiting for approval',
      AppStrings.clientData: 'Client Data',
      AppStrings.newClient: 'New Client',
      AppStrings.requestedAppointment: 'Requested Appointment',
      AppStrings.implementationDurationInDays: 'Implementation Duration (Days)',
      AppStrings.offerExpiryDate: 'Offer Expiry Date',
      AppStrings.offerValue: 'Offer Value (OMR)',
      AppStrings.offerDetails: 'Offer Details...',
      AppStrings.submitOffer: 'Submit Offer',
      AppStrings.offerExpiresAt: 'Offer expires at a specific time',
      AppStrings.offerSentSuccess: 'Offer sent successfully',
      AppStrings.sendOfferNow: 'Send Offer Now',
      AppStrings.following: 'Following',
      AppStrings.follow: 'Follow',
      AppStrings.showMore: 'Show More',
      AppStrings.showLess: 'Show Less',
      AppStrings.omani: 'Omani',
      AppStrings.primaryDegree: 'Primary',
      AppStrings.appealDegree: 'Appeal',
      AppStrings.supremeDegree: 'Supreme / Cassation',
      AppStrings.specCriminalShort: 'Criminal',
      AppStrings.specCivilShort: 'Civil',
      AppStrings.specPersonalStatusShort: 'Personal Status',
      AppStrings.specCorporateShort: 'Corporate',
      AppStrings.specOtherShort: 'Other',
      AppStrings.serviceLegalConsultation: 'Legal Consultation',
      AppStrings.serviceLegalRepresentation: 'Legal Representation',
      AppStrings.serviceContractWriting: 'Contract Writing',
      AppStrings.errorFetchBookings: 'Error fetching bookings',
      AppStrings.errorFetchCases: 'Error fetching cases',
      AppStrings.errorFetchClients: 'Error fetching clients',
      AppStrings.errorFetchStats: 'Error fetching statistics',
      AppStrings.errorFetchTransactions: 'Error fetching transactions',
      AppStrings.errorUpdateStatus: 'Error updating status',
      AppStrings.errorUpdateAvailability: 'Error updating availability',
      AppStrings.muscat: 'Muscat',
      AppStrings.salalah: 'Salalah',
      AppStrings.sohar: 'Sohar',
      AppStrings.nizwa: 'Nizwa',
      AppStrings.sur: 'Sur',
      AppStrings.rustaq: 'Rustaq',
      AppStrings.buraimi: 'Buraimi',
      AppStrings.ibra: 'Ibra',
      AppStrings.haima: 'Haima',
      AppStrings.khasab: 'Khasab',
      AppStrings.seeb: 'Seeb',
      AppStrings.bawshar: 'Bawshar',
      AppStrings.mutrah: 'Mutrah',
      AppStrings.amerat: 'Al Amerat',
      AppStrings.barka: 'Barka',
      AppStrings.ibri: 'Ibri',
      AppStrings.musannah: 'Al Musannah',
      AppStrings.bahla: 'Bahla',
      AppStrings.suwaiq: 'Al Suwaiq',
      AppStrings.arabic: 'Arabic',
      AppStrings.english: 'English',
      AppStrings.saudiArabia: 'Saudi Arabia',
      AppStrings.areYouALawyer: 'Are you a lawyer? ',
      AppStrings.registerAsNewLawyer: 'Register as a new lawyer',
      AppStrings.errorRegistrationFailed: 'Registration failed',
      AppStrings.errorResetPasswordFailed: 'Reset password failed',
      AppStrings.errorVerificationFailed: 'Verification failed',
      AppStrings.errorResendOtpFailed: 'Failed to resend activation code',
      AppStrings.errorServer: 'Server error, please try again later',
      AppStrings.noInternetConnection:
          'No internet connection. Please check your network and try again.',
      AppStrings.requestTimedOut:
          'The request took too long. Please try again.',
      AppStrings.unexpectedError:
          'An unexpected error occurred. Please try again.',
      AppStrings.operationCancelled: 'The operation was cancelled.',
      AppStrings.otpResentSuccessfully: 'OTP resent successfully',
      AppStrings.sessionAddedSuccessfully: 'Session added successfully',
      AppStrings.documentUploadedSuccessfully: 'Document uploaded successfully',
      AppStrings.withdrawalRequestSubmittedSuccessfully:
          'Withdrawal request submitted successfully',
      AppStrings.fixed: 'Fixed',
      AppStrings.goToDashboard: 'Go to Dashboard',
      AppStrings.searchByLawOrLegislation:
          'Search for legal text or legislation...',
      AppStrings.legalCategories: 'Legal Categories',
      AppStrings.latestReferences: 'Latest References',
      AppStrings.criminalLaw: 'Criminal Law',
      AppStrings.commercialLaw: 'Commercial Law',
      AppStrings.personalStatus: 'Personal Status',
      AppStrings.laborLaw: 'Labor Law',
      AppStrings.civilLaw: 'Civil Law',
      AppStrings.omanLaws: 'Oman Laws',
      AppStrings.documentsAndFiles: 'Documents and Files',
      AppStrings.recentFolders: 'Recent Folders',
      AppStrings.latestFiles: 'Latest Files',
      AppStrings.folderName: 'Folder Name',
      AppStrings.openDocument: 'Open',
      AppStrings.addTask: 'Add Task',
      AppStrings.taskTitle: 'Task Title',
      AppStrings.selectPriority: 'Select Priority',
      AppStrings.high: 'High',
      AppStrings.medium: 'Medium',
      AppStrings.low: 'Low',
      AppStrings.legalArticles: 'Legal Articles',
      AppStrings.views: 'views',
      AppStrings.createdAt: 'Published on',
      AppStrings.notifications: 'Notifications',
      AppStrings.dueDate: 'Due Date',
      AppStrings.uploadNewDocument: 'Upload New Document',
      AppStrings.cancel: 'Cancel',
      AppStrings.filesCount: '{count} Files',
      AppStrings.filter: 'Filter',
      AppStrings.requestWithdrawal: 'Request Withdrawal',
      AppStrings.clients: 'Clients',
      AppStrings.consultationsTab: 'Consultations',
      AppStrings.more: 'More',
      AppStrings.totalBookings: 'Total Bookings',
      AppStrings.nextAppointment: 'Next Appointment',
      AppStrings.documents: 'Documents',
      AppStrings.noResults: 'No Results Found',
      AppStrings.noResultsSubtitle:
          'Try searching for another word or changing the search query.',
      AppStrings.myCases: 'Cases',
      AppStrings.mySubscription: 'My Subscriptions',
      AppStrings.serviceName: 'Service Name',
      AppStrings.client: 'Client',
      AppStrings.date: 'Date',
      AppStrings.startSession: 'Start Session',
      AppStrings.confirmed: 'Confirmed',
      AppStrings.otpSentMessage: 'OTP code has been sent to your phone',
      AppStrings.backToLogin: 'Back to Login',
      AppStrings.resetPassword: 'Reset Password',
      AppStrings.passwordChanged: 'Password changed successfully',
      AppStrings.otpCode: 'Verification Code',
      AppStrings.otpMustBe4: 'OTP must be 6 digits',
      AppStrings.invalidOtp: 'Please enter a valid 6-digit code',
      AppStrings.mobileNumber: 'Mobile Number',
      AppStrings.genderTitle: 'Gender',
      AppStrings.gender: 'Gender',
      AppStrings.accountTierTitle: 'Account Type',
      AppStrings.changePassword: 'Change Password',
      AppStrings.profileUpdatedSuccessfully: 'Profile updated successfully',
      AppStrings.logoutTitle: 'Logout',
      AppStrings.logoutSubtitle: 'Are you sure you want to logout?',
      AppStrings.searchHint: 'Search here...',
      AppStrings.hello: 'Hello',
      AppStrings.changeLanguage: 'Change Language',
      AppStrings.support: 'Support',
      AppStrings.back: 'Back',
      AppStrings.next: 'Next',
      AppStrings.retry: 'Retry',
      AppStrings.weAreHereToHelp: 'We are here to help you',
      AppStrings.supportTeamReady:
          'Our support team is ready to answer your inquiries 24/7.',
      AppStrings.phoneCall: 'Phone Call',
      AppStrings.workingHours: 'Working Hours',
      AppStrings.error: 'Error',
      AppStrings.loginFailed: 'Login Failed',
      AppStrings.errorUnrecognized: 'Unrecognized Error',
      AppStrings.loginWithGoogle: 'Login with Google',
      AppStrings.registerWithGoogle: 'Register with Google',
      AppStrings.fingerprintLogin: 'Fingerprint Login',
      AppStrings.transactionHistory: 'Transaction History',
      AppStrings.rateAppStore: 'Rate App',
      AppStrings.dataPrivacy: 'Data Privacy',
      AppStrings.emailNotFound: 'Email not found',
      AppStrings.aboutUsDescription:
          'hogga is a platform that connects clients with licensed lawyers easily and securely.',
      AppStrings.ourGoals: 'Our Goals',
      AppStrings.ourGoalsContent:
          'We aim to facilitate access to justice and legal support for everyone.',
      AppStrings.linkBetweenUserAndProvider:
          'We are the link between you and the legal experts.',
      AppStrings.aboutUsContent:
          'Our team is dedicated to providing high-quality legal consultations through modern technology.',
      AppStrings.privacyIntroTitle: 'Introduction',
      AppStrings.privacyIntroContent:
          'We care about your privacy and protect your data according to international standards.',
      AppStrings.dataWeCollectTitle: 'Data Collection',
      AppStrings.dataWeCollectContent:
          'We collect data necessary to provide and improve our services.',
      AppStrings.paymentDataNotice:
          'Payment data is processed securely through licensed partners.',
      AppStrings.howWeUseDataTitle: 'Data Usage',
      AppStrings.howWeUseDataContent:
          'Your data is used to match you with lawyers and manage your cases.',
      AppStrings.dataSharingTitle: 'Data Sharing',
      AppStrings.dataSharingContent:
          'We only share data with lawyers you choose to work with.',
      AppStrings.dataProtectionTitle: 'Data Protection',
      AppStrings.dataProtectionContent:
          'We use advanced encryption to protect your personal information.',
      AppStrings.userRightsTitle: 'Your Rights',
      AppStrings.userRightsContent:
          'You have the right to access, correct, or delete your data at any time.',
      AppStrings.policyUpdatesTitle: 'Policy Updates',
      AppStrings.policyUpdatesContent:
          'We may update this policy occasionally and will notify you of changes.',
      AppStrings.firstUserSection: 'User Responsibilities',
      AppStrings.userObligations:
          'Users must provide accurate information and follow platform rules.',
      AppStrings.secondProviderSection: 'Lawyer Responsibilities',
      AppStrings.providerObligations:
          'Lawyers must provide professional advice and maintain confidentiality.',
      AppStrings.thirdGeneralSection: 'General Provisions',
      AppStrings.generalProvisions:
          'These guidelines explain how to use the hogga platform safely and correctly.',
      AppStrings.mainFeatures: 'Main Features',
      AppStrings.searchNearby: 'Search Nearby Lawyers',
      AppStrings.realReviews: 'Real Client Reviews',
      AppStrings.reliableSecure: 'Reliable and Secure',
      AppStrings.roundClockSupport: '24/7 Support',
      AppStrings.rateApp: 'Rate hogga',
      AppStrings.copyright: 'All rights reserved © 2026',
      AppStrings.cancellationPolicy: 'Cancellation Policy',
      AppStrings.payment: 'Payment',
      AppStrings.paymentMethod: 'Payment Method',
      AppStrings.total: 'Total',
      AppStrings.confirm: 'Confirm',
      AppStrings.profileTitle: 'Profile',
      AppStrings.working: 'Working',
      AppStrings.add: 'Add',
      AppStrings.currency: 'OMR',
      AppStrings.followUsOnSocialMedia: 'Follow us on social media',
      AppStrings.whyZoneApp: 'Why hogga?',
      AppStrings.version: 'Version 1.0.0',
      AppStrings.cancellationPolicyTitle: 'Cancellation Policy',
      AppStrings.cancellationPolicyContent:
          'You can cancel your request before it is accepted by a lawyer.',
      AppStrings.publicOpportunities: 'Public Opportunities',
      AppStrings.myServices: 'My Services',
      AppStrings.mySpecializations: 'My Specializations',
      AppStrings.addService: 'Add Service',
      AppStrings.editService: 'Edit Service',
      AppStrings.serviceType: 'Service Type',
      AppStrings.price: 'Price',
      AppStrings.serviceDescription: 'Service Description',
      AppStrings.submitProposal: 'Submit Proposal',
      AppStrings.proposalDescription: 'Proposal Details',
      AppStrings.proposalPrice: 'Proposal Price',
      AppStrings.deliveryTime: 'Delivery Time',
      AppStrings.days: 'Days',
      AppStrings.noOpportunities: 'No public requests available at the moment.',
      AppStrings.noServices: 'You haven\'t added any services yet.',
      AppStrings.serviceAddedSuccess: 'Service added successfully',
      AppStrings.proposalSubmittedSuccess: 'Proposal submitted successfully',
      AppStrings.lawyerDashboardTitle: 'Lawyer Dashboard',
      AppStrings.professionalTools: 'Workspace',
      AppStrings.myProposals: 'My Proposals',
      AppStrings.noProposals: 'No Proposals',
      AppStrings.noProposalsSubtitle:
          'You haven\'t submitted any proposals yet.',
      AppStrings.caseNumberLabel: 'Case Number',
      AppStrings.yourOffer: 'Your Offer',
      AppStrings.currencyRial: 'OMR',
      AppStrings.priceRange: 'Price Range',
      AppStrings.deleteProposal: 'Delete Proposal',
      AppStrings.confirmDeleteProposal:
          'Are you sure you want to delete this proposal?',
      AppStrings.cancelProposal: 'Withdraw Proposal',
      AppStrings.confirmCancelProposal:
          'Are you sure you want to withdraw this proposal?',
      AppStrings.editProposal: 'Edit Proposal',
      AppStrings.optionalDocuments: 'Documents (Optional)',
      AppStrings.addFile: 'Add File',
      AppStrings.accepted: 'Accepted',
      AppStrings.acceptedProposal: 'Proposal Accepted',
      AppStrings.rejected: 'Rejected',
      AppStrings.pendingReview: 'Pending Review',
      AppStrings.operationSuccess: 'Operation successful',
      AppStrings.proposalsCountLabel: '{count} Proposals',
      AppStrings.pleaseSelectSessionDate: 'Please select session date',
      AppStrings.sessionTitleHint: 'Session Title',
      AppStrings.sessionDateLabel: 'Session Date',
      AppStrings.sessionDetailsHint: 'Session Details',
      AppStrings.pleaseSelectFile: 'Please select a file to upload',
      AppStrings.documentTitleHint: 'Document Title',
      AppStrings.clickToSelectFile: 'Click to select file (PDF, PNG, JPG)',
      AppStrings.byClient: 'By Client',
      AppStrings.byLawyer: 'By Lawyer',
      AppStrings.active: 'Active',
      AppStrings.paused: 'Paused',
      AppStrings.inCourts: 'In Courts',
      AppStrings.contractWriting: 'Contract Writing',
      AppStrings.remoteConsultation: 'Remote Consultation',
      AppStrings.manageSchedule: 'Manage Schedule',
      AppStrings.myClients: 'Clients',
      AppStrings.pleaseCompleteAllFields: 'Please complete all fields',
      AppStrings.selectionType: 'Selection Type',
      AppStrings.paymentStatus: 'Payment Status',
      AppStrings.discount: 'Discount',
      AppStrings.receivedProposals: 'Received Proposals',
      AppStrings.confirmAcceptProposal:
          'Are you sure you want to accept the proposal from {name}?',
      AppStrings.enterDocumentTitles: 'Enter title for each document',
      AppStrings.documentTitle: 'Document Title',
      AppStrings.pleaseEnterDocumentTitles:
          'Please enter a title for each document',
      AppStrings.pleaseSelectExecutionDate:
          'Please select execution date and time',
      AppStrings.pleaseSelectAtLeastOneLawyer:
          'Please select at least one lawyer',
      AppStrings.executionDateTime: 'Execution Date & Time',
      AppStrings.chooseExecutionDateTime: 'Choose execution date and time',
      AppStrings.couponApplied: 'Coupon applied: {code}',
      AppStrings.discountLabel: 'Discount:',
      AppStrings.submitting: 'Submitting...',
      AppStrings.budgetRange: 'Budget Range',
      AppStrings.minPrice: 'Min Price',
      AppStrings.maxPrice: 'Max Price',
      AppStrings.pleaseEnterMaxPrice: 'Please enter a maximum price',
      AppStrings.newRequest: 'New Request',

      // Lawyer Proposals & Services missing translations
      AppStrings.services: 'Services',
      AppStrings.clientRequests: 'Client Requests',
      AppStrings.availableServices: 'Available Services',
      AppStrings.submitPriceOffer: 'Submit Proposal',
      AppStrings.acceptTextConsultations: 'Text Consults',
      AppStrings.acceptScheduledConsultations: 'Scheduled Consults',
      AppStrings.manageConsultationPrices: 'Manage Consultation Prices',
      AppStrings.consultationType: 'Consultation Type',
      AppStrings.addPriceOption: 'Add Price Option',
      AppStrings.savePrices: 'Save Prices',
      AppStrings.durationMinutes: 'Duration (minutes)',
      AppStrings.priceAmount: 'Price Amount',
      AppStrings.totalBalance: 'Total Balance',
      AppStrings.totalSent: 'Total Sent',
      AppStrings.totalPurchases: 'Total Purchases',
      AppStrings.pendingPurchases: 'Pending Purchases',
      AppStrings.completedPurchases: 'Completed Purchases',
      AppStrings.totalIncome: 'Total Income',
      AppStrings.serviceDetails: 'Service Details',
      AppStrings.activeLabel: 'Active',
      AppStrings.inactiveLabel: 'Inactive',
      AppStrings.confirmDelete: 'Are you sure you want to delete this service?',
      AppStrings.applicationSentSuccess: 'Application sent successfully',
      AppStrings.goToLawyerDashboard: 'Go to Lawyer Dashboard',
      AppStrings.chooseAccountType: 'Choose Account Type',
      AppStrings.chooseAccountTypeSubtitle:
          'Select the account type that suits you.',
      AppStrings.applicationUnderReviewNotice:
          'Registration completed. Your account is now awaiting administration approval before full activation.',

      AppStrings.personalPhoto: 'Personal Photo',
      AppStrings.photoUploadDesc:
          'Upload a clear photo in PNG, JPG, or WEBP format.',
      AppStrings.nameRequired: 'Name is required',
      AppStrings.validNameRequired: 'Please enter a valid full name.',
      AppStrings.civilId: 'Civil ID',
      AppStrings.expRequired: 'Years of experience is required',
      AppStrings.validExpRequired: 'Please enter a valid number',
      AppStrings.alreadyHaveAccountLogin: 'Already have an account? Login',
      AppStrings.additionalDataTitle: 'Complete Additional Info',
      AppStrings.additionalDataSubtitle:
          'These fields are determined by the server based on your account type. Fields marked with an asterisk are required.',
      AppStrings.submitAndCompleteRegistration:
          'Submit & Complete Registration',
      AppStrings.allowedFilesDesc:
          'Allowed files: PDF, JPG, PNG, WEBP up to 5MB.',
      AppStrings.confirmAccept: 'Confirm Acceptance',
      AppStrings.confirmRefuse: 'Confirm Refusal',
      AppStrings.areYouSure:
          'Are you sure you want to proceed with this action?',
      AppStrings.rateLawyer: 'Rate Lawyer',
      AppStrings.rateLawyerTitle: 'Rate {name}',
      AppStrings.rateLawyerSubtitle:
          'Share your experience to help other clients choose confidently.',
      AppStrings.ratingComment: 'Comment',
      AppStrings.ratingCommentHint: 'Write your feedback here...',
      AppStrings.submitRating: 'Submit Rating',
      AppStrings.ratingSentSuccessfully: 'Rating submitted successfully',
      AppStrings.uploadFile: 'Upload {name}',
      AppStrings.chooseFileHint: 'Choose the appropriate file for this field',
      AppStrings.enterNumberHint: 'Enter {name} number',
      AppStrings.enterTextHint: 'Enter {name}',
      AppStrings.required: 'Required',
      AppStrings.optional: 'Optional',
      AppStrings.chooseSpecsSubtitle:
          'You can choose more than one specialization.',
      AppStrings.specsSelectedCount: '{count} specializations selected',
      AppStrings.verificationCodeTitle: 'Phone Verification',
      AppStrings.verificationCodeHint: 'Verification Code',
      AppStrings.verifyCodeButton: 'Confirm Code',
      AppStrings.enterVerificationCode:
          'Enter the verification code sent to {phone}',
      AppStrings.browseOpportunities: 'Browse Available Opportunities',
      AppStrings.noServicesSubtitle:
          'You haven\'t added any legal services yet. Start now by adding your first service.',
      AppStrings.addNewService: 'Add New Service',
      AppStrings.legalRepresentation: 'Legal Representation',
      AppStrings.companyFormation: 'Company Formation',
      AppStrings.pleaseSelectServiceType: 'Please select a service type',
      AppStrings.markAllAsRead: 'Mark all as read',
      AppStrings.noNotifications: 'No notifications',
      AppStrings.noNotificationsSubtitle:
          'We will notify you when there are new updates.',
      AppStrings.callNoLongerAvailable: 'This call is no longer available.',
      AppStrings.minutesAgo: '{count} minutes ago',
      AppStrings.hoursAgo: '{count} hours ago',
      AppStrings.daysAgo: '{count} days ago',
      AppStrings.chooseLawyer: 'Choose Lawyer',
      AppStrings.uploadDocumentsDesc:
          'Upload required documents (PDF, JPG, PNG)',
      AppStrings.browseFiles: 'Browse Files',
      AppStrings.broadcastDesc:
          'Your request will reach all available lawyers within minutes',
      AppStrings.chooseLawyerManually: 'Choose Lawyers Manually',
      AppStrings.manualChooseDesc:
          'Select your preferred lawyers to send your request to them directly',

      AppStrings.stepPlatformFees: 'Platform Fees',
      AppStrings.payByCard: 'Pay by Card',
      AppStrings.payByWallet: 'Pay by Wallet',
      AppStrings.promoCode: 'Promo Code',
      AppStrings.servicePrice: 'Service Price',
      AppStrings.tax: 'Tax',
      AppStrings.confirmAndPay: 'Confirm & Pay',

      AppStrings.selected: 'Selected',
      AppStrings.browseLawyers: 'Browse Lawyers',
      AppStrings.lawyersReadyDesc:
          'More than 100 lawyers ready to respond to you',
      AppStrings.serviceNameLabel: 'Service',
      AppStrings.securePayment: 'Secure',

      AppStrings.uploadDocuments: 'Upload Documents',
      AppStrings.subscriptionPackages: 'Subscription Packages',
      AppStrings.currentSubscription: 'Current Subscription',
      AppStrings.consultationLeft: 'Consultations Left',
      AppStrings.choosePackage: 'Choose a Package',
      AppStrings.notVisibleToClients: 'Profile not visible to clients',
      AppStrings.activateSubscriptionToStart:
          'Please activate a subscription to start receiving requests',
      AppStrings.subscriptionExpiringSoon: 'Subscription expiring soon',
      AppStrings.renew: 'Renew',
      AppStrings.visibleToClients: 'Profile visible to clients',
      AppStrings.manage: 'Manage',
      AppStrings.defaultFreePackage: 'Default free package',
      AppStrings.remainingDays: 'Remaining days',
      AppStrings.noExpiryDate: 'No expiry date',
      AppStrings.packages: 'Packages',
      AppStrings.activateNow: 'Activate Now',
      AppStrings.upgradePackage: 'Upgrade Package',
      AppStrings.subscriptionSuccess: 'Subscribed successfully',
      AppStrings.notActive: 'Not Active',
      AppStrings.pending: 'Pending',
      AppStrings.refresh: 'Refresh',
      AppStrings.willNotifyYou:
          'We will notify you once your account is approved.',
      AppStrings.pressBackAgainToExit: 'Press back again to exit',
      AppStrings.welcomeTo: 'Welcome to',
      AppStrings.priceExceedsBudget: 'Price cannot exceed client budget',
      AppStrings.userRoleDescription:
          'Search for lawyers and get legal consultations',
      AppStrings.lawyerRoleDescription:
          'Manage your cases and reach new clients',
      AppStrings.settings: 'Settings',
      AppStrings.draftRestored: 'Previous draft restored successfully',
      AppStrings.pleaseSelectAccountType: 'Please select account type',
      AppStrings.pleaseSelectAtLeastOneSpec:
          'Please select at least one specialization',
      AppStrings.pleaseUploadPersonalPhoto: 'Please upload personal photo',
      AppStrings.pleaseUploadField: 'Please upload {field}',
      AppStrings.pleaseEnterField: 'Please enter {field}',
      AppStrings.unsupportedFileFormat: 'Unsupported file format',
      AppStrings.fileSizeTooLarge: 'File size is too large',
      AppStrings.stepXofY: 'Step {current} of {total}',
      AppStrings.verificationCodeSent: 'Verification code sent successfully',
      AppStrings.phoneVerifiedSuccess: 'Phone verified successfully',
      AppStrings.selectAccountTypeFirst: 'Please select account type first',
      AppStrings.basicInfoSaved: 'Basic information saved successfully',
      AppStrings.phoneAlreadyRegistered:
          'This phone number is already registered',
      AppStrings.emailAlreadyRegistered: 'This email is already registered',
      AppStrings.noOpportunitiesSubtitle:
          'No customer requests available in your specializations. We will notify you once new opportunities arise.',
      AppStrings.emptyServicesHistory:
          'Available services history is currently empty.',
      AppStrings.noActiveOrdersSubtitle:
          'You don\'t have any active orders currently. You can browse services and request a new consultation.',
      AppStrings.emptyOrderHistory: 'Order history is currently empty.',
      AppStrings.maintenanceDefaultMessage:
          'We are performing some improvements. We will be back soon.',
      AppStrings.updateDefaultMessage: 'Please update the app to continue.',
      AppStrings.processingAndVerifying: 'Processing and verifying file...',
      AppStrings.fileVerifiedSuccess: 'File verified successfully',
      AppStrings.welcome: 'Welcome',
      AppStrings.fullName: 'Full Name',
      AppStrings.addCaseUpdate: 'Add Case Update',
      AppStrings.clientDetails: 'Client Details',
      AppStrings.noDataFound: 'No data found',

      AppStrings.deleteTask: 'Delete Task',
      AppStrings.confirmDeleteTask:
          'Are you sure you want to delete this task?',
      AppStrings.noTasks: 'No Tasks',
      AppStrings.noTasksSubtitle: 'You haven\'t added any tasks yet.',
      AppStrings.documentName: 'Document Name',
      AppStrings.upload: 'Upload',
      AppStrings.noFileSelected: 'No file selected',
      AppStrings.deleteDocument: 'Delete Document',
      AppStrings.confirmDeleteDocument:
          'Are you sure you want to delete this document?',
      AppStrings.pendingBalance: 'Pending Balance',
      AppStrings.amount: 'Amount',
      AppStrings.accountName: 'Account Name',
      AppStrings.bankName: 'Bank Name',
      AppStrings.iban: 'IBAN',
      AppStrings.sendRequest: 'Send Request',
      AppStrings.viewDetails: 'View Details',

      AppStrings.apply: 'Apply',
      AppStrings.edit: 'Edit',
      AppStrings.delete: 'Delete',
      AppStrings.servicesAndProducts: 'Services and Products',
      AppStrings.quantity: 'Quantity',
      AppStrings.paymentDetails: 'Payment Details',
      AppStrings.paidAmount: 'Paid Amount',
      AppStrings.transactionNumber: 'Transaction Number',
      AppStrings.invoiceNumber: 'Invoice Number',
      AppStrings.chats: 'Chats',
      AppStrings.noChats: 'No Chats',
      AppStrings.noLawyerChatsSubtitle:
          'Your chats with lawyers will appear here',
      AppStrings.noClientChatsSubtitle:
          'Your chats with clients will appear here',
      AppStrings.startConversation: 'Start Conversation',
      AppStrings.calling: 'Calling...',
      AppStrings.waitingForLawyer: 'Waiting for lawyer to join...',
      AppStrings.waitingForClient: 'Waiting for client to join...',
      AppStrings.connected: 'Connected',
      AppStrings.switchCamera: 'Switch Camera',
      AppStrings.speaker: 'Speaker',
      AppStrings.mute: 'Mute',
      AppStrings.video: 'Video',
      AppStrings.end: 'End',
      AppStrings.failedToSendMessage: 'Failed to send message',
      AppStrings.failedToSendFile: 'Failed to send file',
      AppStrings.voiceCall: 'Call',
      AppStrings.startConversationWithLawyer: 'Start conversation with lawyer',
      AppStrings.startConversationWithClient: 'Start conversation with client',
      AppStrings.typeYourMessage: 'Type your message...',
      AppStrings.pdfFile: 'PDF File',
      AppStrings.attachment: 'Attachment',
      AppStrings.upcomingSessions: 'Upcoming Sessions',
      AppStrings.previousSessions: 'Previous Sessions',
      AppStrings.addAction: 'Add',
      AppStrings.completePaymentNow: 'Complete Payment Now',
      AppStrings.paymentScreenTitle: 'Complete Payment',
      AppStrings.paymentFailedDialogTitle: 'Payment was not completed',
      AppStrings.paymentFailedDialogMessage:
          'There was a problem completing the payment. You can try again or return to the home screen.',
      AppStrings.cancelPaymentDialogTitle:
          'Are you sure you want to cancel the payment?',
      AppStrings.cancelPaymentDialogMessage:
          'The current payment process will be stopped and you will return to the home screen.',
      AppStrings.continuePayment: 'Continue Payment',
      AppStrings.paymentLinkUnavailable:
          'Payment link is not available yet. Please try again.',
      AppStrings.makeVideoCall: 'Start Call',
      AppStrings.makeVoiceCall: 'Start Call',
      AppStrings.enterChat: 'Enter Chat',
      AppStrings.chat: 'Chat',
      AppStrings.paidStatus: 'Paid',
      AppStrings.imagePreview: 'Image',
      AppStrings.audioPreview: 'Audio File',
      AppStrings.referralCode: 'Referral Code',
      AppStrings.referralCodeOptional: 'Referral Code (Optional)',
      AppStrings.enterReferralCodeHint: 'Enter referral code if any',
      AppStrings.referralProgram: 'Referral Program',
      AppStrings.referralHistory: 'Referral History',
      AppStrings.referralCampaignTitle: 'Client & Lawyer Referral Bonus',
      AppStrings.referralCampaignSubtitle:
          'Share your referral code and earn {amount} {currency} per successful registration',
      AppStrings.copyCode: 'Copy Code',
      AppStrings.codeCopiedSuccess: 'Referral code copied to clipboard',
      AppStrings.totalReferredUsers: 'Total Referred Users',
      AppStrings.referralBonus: 'Referral Bonus',
      AppStrings.referralBonusFromReferrals: 'Referral Earnings',
      AppStrings.rewarded: 'Rewarded',
      AppStrings.notRewarded: 'Pending/Not Rewarded',
      AppStrings.invalidReferralCode: 'Invalid referral code',
      AppStrings.noReferralsYet: 'No referrals yet',
      AppStrings.noReferralsYetSubtitle:
          'Share your referral code with new users to earn bonuses',
      AppStrings.shareCodePrompt:
          'Share this code with new clients upon registration to earn financial rewards.',
      AppStrings.referredUser: 'Referred User',
      AppStrings.registrationDate: 'Registration Date',
      AppStrings.rewardAmount: 'Reward Amount',
      AppStrings.details: 'Details',
      AppStrings.waitingForProposals: 'Waiting for Proposals',
      AppStrings.waitingForProposalsSubtitle:
          'Your request is currently visible to lawyers. Proposals will appear here as soon as they are submitted.',
      AppStrings.acceptRequestTitle: 'Accept Request & Set Price',
      AppStrings.requestAcceptedSuccessfully:
          'Request accepted and offer submitted successfully',
    },
    'ar': {
      AppStrings.appName: 'حُجّة',
      AppStrings.noRouteFound: 'لم يتم العثور على المسار',
      AppStrings.login: 'تسجيل الدخول',
      AppStrings.register: 'إنشاء حساب جديد',
      AppStrings.email: 'البريد الإلكتروني',
      AppStrings.password: 'كلمة المرور',
      AppStrings.confirmPassword: 'تأكيد كلمة المرور',
      AppStrings.forgotPassword: 'نسيت كلمة المرور؟',
      AppStrings.sendResetLink: 'إرسال رابط إعادة التعيين',
      AppStrings.resetEmailSent: 'تم إرسال رابط إعادة تعيين كلمة المرور',
      AppStrings.profile: 'حسابي',
      AppStrings.requiredField: 'هذا الحقل مطلوب',
      AppStrings.passwordMismatch: 'كلمة المرور غير متطابقة',
      AppStrings.invalidPhone: 'رقم هاتف غير صحيح',
      AppStrings.invalidEmail: 'بريد إلكتروني غير صحيح',
      AppStrings.passwordTooShort: 'يجب أن تكون كلمة المرور 8 أحرف على الأقل',
      AppStrings.invalidCivilID: 'رقم هويّة غير صحيح (8 أرقام)',
      AppStrings.invalidLawyerLicense: 'رقم قيد غير صحيح',
      AppStrings.registerAsUser: 'إنشاء حساب مستخدم',
      AppStrings.language: 'اللغة',
      AppStrings.aboutUs: 'عن حُجّة',
      AppStrings.aboutApp: 'عن التطبيق',
      AppStrings.contactUs: 'تواصل معنا',
      AppStrings.privacyPolicy: 'سياسة الخصوصية',
      AppStrings.logout: 'تسجيل الخروج',
      AppStrings.instructions: 'إرشادات الاستخدام',
      AppStrings.lawyerWallet: 'المحفظة',
      AppStrings.freeConsultations: 'الاستشارات المجانية',
      AppStrings.yourConsultationPercentage:
          'نسبتك من الاستشارة: {percentage}%',
      AppStrings.used: 'المستخدمة',
      AppStrings.remaining: 'المتبقية',
      AppStrings.campaignNotActive: 'الحملة غير مفعلة حالياً',
      AppStrings.rewardValue: 'قيمة المكافأة',
      AppStrings.referralCodeTitle: 'كود الإحالة',
      AppStrings.codeCopied: 'تم نسخ الكود بنجاح',
      AppStrings.bookings: 'الحجوزات',

      AppStrings.termsAgreementIntro: 'بإنشائك للحساب، فأنت توافق على',
      AppStrings.and: 'و',
      AppStrings.termsOfUseTitle: 'إرشادات الاستخدام',
      AppStrings.welcomeTitle: 'ابدأ رحلتك مع حُجَّة',
      AppStrings.welcomeSubtitle: 'اختر نوع الحساب للمتابعة',
      AppStrings.userCardSubtitle: 'استشارات قانونية وأسرية موثوقة',
      AppStrings.lawyerCardSubtitle: 'تقديم الاستشارات والخدمات القانونية',
      AppStrings.registerAsLawyer: 'إنشاء حساب استشاري',
      AppStrings.myOrders: 'القضايا',
      AppStrings.noOrdersYet: 'لا توجد قضايا حتى الآن',
      AppStrings.loginSubtitle: 'أهلاً بك مجدداً',
      AppStrings.dontHaveAccount: 'ليس لديك حساب؟ ',
      AppStrings.alreadyHaveAccount: 'لديك حساب بالفعل؟ ',
      AppStrings.male: 'ذكر',
      AppStrings.female: 'أنثى',
      AppStrings.agreeToTermsError: 'يرجى الموافقة على إرشادات الاستخدام',
      AppStrings.individual: 'أفراد',
      AppStrings.corporate: 'شركة',
      AppStrings.institution: 'مؤسسة',
      AppStrings.iAgreeTo: 'أوافق على ',
      AppStrings.chooseCountry: 'اختار البلد',
      AppStrings.accountTier: 'نوع الحساب',
      AppStrings.fullNameHint: 'الاسم بالكامل',
      AppStrings.phoneStepTitle: 'رقم الجوال',
      AppStrings.nameStepTitle: 'تسجيل الاسم',
      AppStrings.passwordStepTitle: 'إنشاء كلمة مرور',
      AppStrings.phoneStepSubtitle: 'أدخل رقم جوالك لإنشاء حساب جديد',
      AppStrings.nameStepSubtitle: 'أكتب اسمك بالكامل',
      AppStrings.passwordStepSubtitle: 'اختر كلمة مرور آمنة',
      AppStrings.createAccountButton: 'إنشاء الحساب',
      AppStrings.activationCode: 'كود التحقق',
      AppStrings.didNotReceiveCode: 'لم تستلم كود التحقق؟',
      AppStrings.resendAfter: 'إعادة الارسال خلال',
      AppStrings.seconds: 'ث',
      AppStrings.resendNow: 'إعادة الإرسال الآن',
      AppStrings.whatsYourPhone: 'ايش رقم جوالك؟',
      AppStrings.areYouLawyer: 'هل انت محامي؟ ',
      AppStrings.emailTaken: 'هذا البريد الإلكتروني مسجل مسبقاً',
      AppStrings.phoneTaken: 'رقم الهاتف هذا مسجل مسبقاً',
      AppStrings.invalidCredentials: 'رقم الهاتف أو كلمة المرور غير صحيحة',
      AppStrings.serverError: 'حدث خطأ ما، يرجى المحاولة لاحقاً',
      AppStrings.sessionExpired: 'انتهت الجلسة، يرجى تسجيل الدخول مرة أخرى',
      AppStrings.sevenMinutesFree: 'لديك 7 دقائق استشارة مجانية',
      AppStrings.availabilityToday:
          'متاحة اليوم بدأ من الساعة 10:00 حتى الساعة 23:00',
      AppStrings.howCanWeHelp: 'كيف يمكننا مساعدتك؟',
      AppStrings.legalConsultationDesc:
          'احصل على استشارة قانونية من محامٍ مرخص لمساعدتك في فهم موقفك القانوني واتخاذ القرار الصحيح.',
      AppStrings.judiciaryExecutionDesc:
          'خدمات متخصصة في القضاء والتنفيذ تشمل كتابة الصحائف واللوائح القانونية، الترافع والتوكيل أمام المحاكم، دراسة القضايا، حضور الجلسات، ومتابعة طلبات التنفيذ.',
      AppStrings.businessServicesDesc:
          'خدمات قانونية لدعم قطاع الأعمال تشمل صياغة ومراجعة العقود والاتفاقيات، إضافةً إلى تسجيل وحماية العلامات التجارية.',
      AppStrings.otherServicesDesc:
          'خدمات قانونية وإدارية متنوعة تشمل مراجعة الدوائر والجهات الحكومية، وتقديم خدمات أخرى مخصصة حسب احتياجات العميل.',
      AppStrings.bannerTitle1: 'استشارتك القانونية تبدأ',
      AppStrings.bannerPrice1: 'من 100 ر.ع فقط!',
      AppStrings.bookNow: 'احجز الآن',
      AppStrings.bannerTitle2: 'استشارة قانونية فورية',
      AppStrings.bannerPrice2: 'مع محامٍ متخصص',
      AppStrings.startNow: 'ابدأ الآن',
      AppStrings.bannerTitle3: 'خدمات قانونية متكاملة',
      AppStrings.bannerPrice3: 'للأفراد والشركات',
      AppStrings.learnMore: 'تعرف أكثر',
      AppStrings.bannerDesc: 'تحدث مع محامي متخصص الآن من خلال مكالمة مباشرة',
      AppStrings.startCallNow: 'بدء الاتصال الآن',
      AppStrings.darkMode: 'الوضع الداكن',
      AppStrings.lightMode: 'الوضع الفاتح',
      AppStrings.home: 'الرئيسية',
      AppStrings.personalInfo: 'المعلومات الشخصية',
      AppStrings.securitySettings: 'إعدادات الأمان',
      AppStrings.editProfile: 'تعديل الملف الشخصي',
      AppStrings.currentPassword: 'كلمة المرور الحالية',
      AppStrings.newPassword: 'كلمة المرور الجديدة',
      AppStrings.confirmNewPassword: 'تأكيد كلمة المرور الجديدة',
      AppStrings.saveChanges: 'حفظ التغييرات',
      AppStrings.changePhoto: 'تغيير الصورة',
      AppStrings.skip: 'تخطي',
      AppStrings.onboardingTitle1: 'تلقَّ عروضاً من أفضل\nالمحامين المرخصين',
      AppStrings.onboardingSubtitle1: 'خدمات واستشارات قانونية\nبمميزات عالية',
      AppStrings.onboardingTitle2: 'استشاراتك القانونية\nفي أي وقت ومكان',
      AppStrings.onboardingSubtitle2:
          'تواصل مع محامٍ متخصص\nعبر المكالمة أو الاجتماع الأونلاين',
      AppStrings.onboardingTitle3: 'تابع قضيتك\nخطوة بخطوة',
      AppStrings.onboardingSubtitle3:
          'ارفع المستندات وتابع مراحل\nقضيتك بشكل كامل وشفاف',
      AppStrings.onboardingLawyer1: 'سلطان البلوشي',
      AppStrings.onboardingLawyer2: 'أحمد الرواحي',
      AppStrings.onboardingSpec1: 'قانون تجاري',
      AppStrings.onboardingSpec2: 'قانون أسري',
      AppStrings.onboardingStatusAvailable: 'متاح',
      AppStrings.onboardingStatusRunning: 'جارية',
      AppStrings.onboardingPrice1: '45 ر.ع',
      AppStrings.onboardingPrice2: '30 ر.ع',
      AppStrings.onboardingDetails: 'التفاصيل',
      AppStrings.onboardingCall: 'مكالمة',
      AppStrings.onboardingInPerson: 'حضوري',
      AppStrings.onboardingOnline: 'أونلاين',
      AppStrings.onboardingCaseNo: 'قضية رقم #3312',
      AppStrings.onboardingStepNew: 'طلب جديد',
      AppStrings.onboardingStepReview: 'قيد المراجعة',
      AppStrings.onboardingStepRunning: 'قيد التنفيذ',
      AppStrings.onboardingStepCompleted: 'مكتمل',
      AppStrings.ongoingConsultations: 'الحالية',
      AppStrings.consultationHistory: 'السابقة',
      AppStrings.noOrdersInSection: 'لا توجد سجلات في هذا القسم',
      AppStrings.hoggaServicesHub: 'خدمات حُجّة',
      AppStrings.chooseSpecialization: 'أختر التخصص',
      AppStrings.legalConsultationsTab: 'الاستشارات القانونية',
      AppStrings.judiciaryExecutionTab: 'القضاء والتنفيذ',
      AppStrings.businessServicesTab: 'خدمات الأعمال',
      AppStrings.otherServicesTab: 'خدمات أخرى',
      AppStrings.immediateConsultation: 'استشارة فورية',
      AppStrings.immediateConsultationDesc:
          'تواصل فورًا مع محامٍ عبر مكالمة. دون الحاجة لحجز موعد مسبق',
      AppStrings.writtenConsultation: 'استشارة كتابية',
      AppStrings.writtenConsultationDesc:
          'تواصل كتابيًا مع محامٍ لطرح استفساراتك وإرسال مستنداتك عبر دردشة نصية، بدون مكالمات.',
      AppStrings.scheduledConsultation: 'استشارة مجدولة',
      AppStrings.scheduledConsultationDesc:
          'احجز استشارة مع محامٍ من اختيارك في الوقت الذي يناسبك، عبر مكالمة.',
      AppStrings.legalWritings: 'كتابات قانونية',
      AppStrings.legalWritingsDesc:
          'صياغة اللوائح والمذكرات الجوابية الاعتراضية وصحائف الدعوى والنقض الالتماس وغيرها من الخطابات بطريقة قانونية سليمة',
      AppStrings.pleadingAndAttorneyship: 'الترافع والتوكيل',
      AppStrings.pleadingAndAttorneyshipDesc:
          'توكيل محامي للترافع عنك في القضية بشكل كامل حتى صدور الحكم الابتدائي او القطعي وتقديم طلب التنفيذ عند الحاجة',
      AppStrings.sessionAttendance: 'حضور جلسة',
      AppStrings.sessionAttendanceDesc:
          'اذا كنت مشغول، حصل لك ظرف او ما عندك رغبة تحضر الجلسة لاي سبب كان. وكل محامي/ة يحضر الجلسة عنك',
      AppStrings.caseStudy: 'دراسة قضية',
      AppStrings.caseStudyDesc:
          'اذا كان موضوعك يحتوي الكثير من التفاصيل والعقود والمرفقات وصكوك الاحكام المطلوب دراستها للوصول للاجابات التي تحتاجها. وتحتاج اجابات دقيقة بخصوص موضوعك.',
      AppStrings.executionRequests: 'طلبات التنفيذ',
      AppStrings.executionRequestsDesc:
          'رفع طلب التنفيذ و/أو متابعة الاجراءات حتى اخر اجراء ضد المنفذ ضده امام محكمة التنفيذ',
      AppStrings.govAgenciesReview: 'مراجعة الجهات والدوائر الحكومية',
      AppStrings.govAgenciesReviewDesc:
          'تحتاج احد يراجع عنك وزارة، نيابة، قسم شرطة، مرور او اي جهة او دائرة حكومية سواء كان في مدينتك او مدينة بعيدة عنك',
      AppStrings.otherLegalServices: 'خدمات اخرى',
      AppStrings.otherLegalServicesDesc:
          'اي خدمات قانونية اخرى ترغب في توكيلها للمحامي او الحصول عليها من محامي. قم بطرح موضوعك هنا وحدد نوع الخدمة المطلوبة',
      AppStrings.specPersonalStatus: 'أحوال شخصية',
      AppStrings.specPersonalStatusDesc:
          'يختص بقضايا الزواج و الخلع وفسخ النكاح والنفقة والحضانة والزيارة والحقوق المتبادلة بين الزوجين.',
      AppStrings.specInheritance: 'إرث وتركات',
      AppStrings.specInheritanceDesc:
          'يختص بقضايا قسمة الإرث والتركات وتنفيذ الوصية والوقف.',
      AppStrings.specCriminal: 'جنائية',
      AppStrings.specCriminalDesc:
          'يختص بكافة القضايا الجنائية ابتداءً بقضايا السب والشتم والقذف والنصب والاحتيال مروراً بجرائم الابتزاز والسرقة والاعتداء وصولاً إلى قضايا القتل والقصاص والدية.',
      AppStrings.specPublicRights: 'حقوق وعامه',
      AppStrings.specPublicRightsDesc:
          'يختص بقضايا المطالبات المالية وتأخر السداد والقضايا المتعلقة بالعقار ( تنازع على ملكية عقار ، تأخر سداد الإيجارات ، رفض إخلاء منزل أو محل ... الخ )',
      AppStrings.specLabor: 'عمالية',
      AppStrings.specLaborDesc:
          'يختص بقضايا الخلافات العمالية بين العامل وصاحب العمل سواء كانت بسبب الأجور أو الفصل أو إصابات العمل أو نهاية الخدمة وغيرها.',
      AppStrings.specCommercial: 'تجارية',
      AppStrings.specCommercialDesc:
          'يختص بقضايا المعاملات التجارية والمنافسة ومنع الاحتكار والتجارة الالكترونية والأسواق المالية والأسهم والمنازعات بين الشركاء والإفلاس وشطب وتصفية السجل التجاري.',
      AppStrings.specIntellectualProperty: 'الملكية الفكرية',
      AppStrings.specIntellectualPropertyDesc:
          'يختص بقضايا براءة الاختراع ، حقوق المؤلف ، انتهاك حقوق الطبع والنشر ، والعلامات التجارية ، والاسرار التجارية.',
      AppStrings.specAdministrative: 'قضايا إدارية',
      AppStrings.specAdministrativeDesc:
          'يختص بقضايا إلغاء القرارات الإدارية والطعن في صحتها ، قضايا التعويض ضد الجهات الحكومية ، والقضايا التأديبية ضد موظفي الدولة.',
      AppStrings.specExecution: 'قضايا التنفيذ',
      AppStrings.specExecutionDesc:
          'تنفيذ الأحكام القضائية ومحاضر الصلح الموثقة والشيكات والسندات لأمر وعقود الايجار الالكترونية',
      AppStrings.specQuasiJudicial: 'اللجان شبه القضائية',
      AppStrings.specQuasiJudicialDesc:
          'يختص بقضايا المخالفات المرورية والجمارك والزكاة والضريبة والتأمينات الاجتماعية والمنازعات المصرفية والتمويلية والمخالفات الإعلامية والصحية.',
      AppStrings.nextStep: 'التالي',
      AppStrings.chooseServiceType: 'اختر نوع الخدمة',
      AppStrings.subtypeLawsuit: 'صحيفة الدعوى',
      AppStrings.subtypeLawsuitDesc:
          'ما يتم تقديمه إلى المحكمة المختصة وفقاً لأحكام قانون الإجراءات للحصول على الحق المدعوم بالسند أو الإثبات',
      AppStrings.subtypeReplyMemo: 'مذكرة رد على الدعوى',
      AppStrings.subtypeReplyMemoDesc:
          'وثيقة قانونية مكتوبة للرد على الدعوى المقامة ضدك امام المحكمة المختصة بعد الاستماع او الاطلاع على الدعوى في الجلسة الاولى',
      AppStrings.subtypeReplyJoinder: 'مذكرة رد جوابية',
      AppStrings.subtypeReplyJoinderDesc:
          'وثيقة قانونية مكتوبة لتبادل الردود والدفاع والشرح بين اطراف القضية او اجابة لطلبات القاضي في الجلسات المنعقدة',
      AppStrings.subtypeObjection: 'لائحة اعتراض',
      AppStrings.subtypeObjectionDesc:
          'وثيقة قانونية مكتوبة للرد على الدعوى المقامة ضدك امام المحكمة المختصة بعد الاستماع او الاطلاع على الدعوى في الجلسة الاولى',
      AppStrings.subtypeCassation: 'لائحة نقض',
      AppStrings.subtypeCassationDesc:
          'لائحة يتم تقديمها للمحكمة العليا لنقض الحكم الذي صدر من محاكم الاستئناف خلال المدة المحددة للنقض من تاريخ صدور الحكم',
      AppStrings.stepSendRequest: 'التفاصيل',
      AppStrings.stepChooseLawyer: 'المحامي',
      AppStrings.stepPlatformFees: 'رسوم المنصة',
      AppStrings.stepPayment: 'الدفع',
      AppStrings.requestTitle: 'عنوان الطلب',
      AppStrings.requestTitleHint: 'يرجى كتابة عنوان للطلب بشكل واضح ومختصر',
      AppStrings.requestDetails: 'تفاصيل الطلب',
      AppStrings.requestDetailsHint:
          'يرجى كتابة تفاصيل الموضوع بشكل واضح ومختصر',
      AppStrings.remoteAttendance: 'هل يمكن الحضور عن بعد؟',
      AppStrings.governorate: 'اسم المحافظة',
      AppStrings.governorateHint: 'أدخل اسم المحافظة',
      AppStrings.city: 'المدينة',
      AppStrings.cityHint: 'أدخل اسم المدينة',
      AppStrings.infoDialogStepLabel: 'إرسال الطلب • 1/3',
      AppStrings.infoDialogHeading: 'في هذه الخطوة أكتب البيانات الأساسية',
      AppStrings.infoDialogBody:
          'اكتب ملخصاً عاماً لطلبك بدون ذكر أسماء أو بيانات حساسة، وتأكد أن المرفقات خالية من أي بيانات شخصية. يمكنك مشاركة التفاصيل الخاصة لاحقاً مع المحامي عند التفاوض أو قبول العرض.',
      AppStrings.infoDialogConfirm: 'لقد فهمت',
      AppStrings.broadcastDesc:
          'سيصل طلبك لجميع المحامين المتاحين للرد خلال دقائق',
      AppStrings.chooseLawyerManually: 'اختر المحامين بنفسك',
      AppStrings.manualChooseDesc:
          'حدد المحامين المفضلين لإرسال طلبك إليهم مباشرة',
      AppStrings.payByCard: 'الدفع بالبطاقة',
      AppStrings.payByWallet: 'الدفع بالمحفظة',
      AppStrings.promoCode: 'كود الخصم',
      AppStrings.servicePrice: 'سعر الخدمة',
      AppStrings.tax: 'الضريبة',
      AppStrings.confirmAndPay: 'تأكيد والدفع',
      AppStrings.total: 'الإجمالي',
      AppStrings.omr: 'ر.ع',
      AppStrings.selected: 'تم الاختيار',
      AppStrings.browseLawyers: 'تصفح المحامين',
      AppStrings.lawyersReadyDesc:
          'أكثر من 100 محامي جاهزون للرد عليك خلال دقائق',
      AppStrings.serviceNameLabel: 'الخدمة',
      AppStrings.securePayment: 'دفع آمن',
      AppStrings.howToSendRequest: 'كيف تود إرسال طلبك؟',
      AppStrings.serviceFee: 'رسوم الخدمة',
      AppStrings.serviceFeeDesc:
          'استقبل عروض أسعار من +1000 محامي مرخص وابدأ التفاوض بسهولة .. اقرأ المزيد',
      AppStrings.broadcastToAll: 'أرسل طلبك لجميع المحامين فوراً',
      AppStrings.broadcastToAllDesc:
          'سيصل طلبك إلى أكثر من 584 محامياً جاهزين للرد خلال دقائق',
      AppStrings.chooseLawyersManually: 'اختر المحامين بنفسك',
      AppStrings.chooseLawyersManuallyDesc:
          'حدد المحامين المفضلين لإرسال طلبك مباشرة إليهم',
      AppStrings.searchLawyer: 'ابحث عن محامي',
      AppStrings.sortBy: 'ترتيب حسب',
      AppStrings.lawyerType: 'محامي/محامية',
      AppStrings.availableLawyers: 'المحامين المتاحين',
      AppStrings.selectOneLawyerOnly: 'اختر محاميًا واحدًا فقط',
      AppStrings.selectLawyerToShowPrices:
          'اختر المحامي أولًا لعرض الأسعار المتاحة',
      AppStrings.verifiedLawyer: 'موثق',
      AppStrings.addAll: 'إضافة الكل',
      AppStrings.addLawyer: '+ إضافة المحامي',
      AppStrings.availableNow: 'متاح الآن',
      AppStrings.yearsExperience: 'سنوات الخبرة',
      AppStrings.requestsCount: 'عدد الطلبات',
      AppStrings.chooseCity: 'اختر المدينة',
      AppStrings.searchCity: 'ابحث عن المدينة',
      AppStrings.havePromoCode: 'لديك كود خصم؟',
      AppStrings.enterPromoCode: 'اكتب كود الخصم هنا',
      AppStrings.applyCode: 'تطبيق',
      AppStrings.payWithCard: 'ادفع باستخدام البطاقة',
      AppStrings.payWithWalet: 'ادفع باستخدام المحافظ',
      AppStrings.serviceValue: 'قيمة الخدمة:',
      AppStrings.taxValue: 'قيمة الضريبة:',
      AppStrings.totalDue: 'المطلوب سداده:',
      AppStrings.paymentDisclaimer:
          'لحماية حقوقك وضمان جودة الخدمة، تأكد من إتمام الدفع داخل المنصة فقط. لسنا مسؤولين عن أي مبالغ تدفع للمحامين خارج المنصة.',
      AppStrings.totalAmount: 'إجمالي المطلوب سداداه',
      AppStrings.completeOrder: 'إتمام الطلب',
      AppStrings.confirmCompleteOrder: 'هل أنت متأكد من إتمام هذا الطلب؟',
      AppStrings.completeService: 'تأكيد اكتمال الخدمة',
      AppStrings.confirmCompleteService: 'هل أنت متأكد أن هذه الخدمة اكتملت؟',
      AppStrings.completeConsultation: 'إنهاء الاستشارة',
      AppStrings.confirmCompleteConsultation:
          'هل أنت متأكد أن هذه الاستشارة اكتملت؟',
      AppStrings.currencySymbol: 'ر.ع',
      AppStrings.orderSentSuccessfully: 'تم إرسال طلبك بنجاح',
      AppStrings.orderProcessingDesc:
          'طلبك الآن قيد المعالجة، سيصلك عروض من المحامين المختارين قريباً.',
      AppStrings.orderNumber: 'رقم الطلب',
      AppStrings.trackOrder: 'متابعة الطلب',
      AppStrings.backToHome: 'العودة للرئيسية',
      AppStrings.startCallSubtitle: 'تواصل صوتي مباشر مع المحامي',
      AppStrings.enterChatRoom: 'دخول غرفة المحادثة',
      AppStrings.enterChatSubtitle: 'تحدث كتابياً مباشرة مع المحامي',
      AppStrings.viewServiceDetails: 'عرض تفاصيل الخدمة',
      AppStrings.viewServiceDetailsSubtitle: 'متابعة كافة تفاصيل ومستندات الطلب',
      AppStrings.consultationConfirmedSuccessfully:
          'تم تأكيد حجز الاستشارة بنجاح!',
      AppStrings.consultationReadyDesc:
          'تم تأكيد استشارتك والمحامي جاهز للتواصل معك الآن.',
      AppStrings.quickActions: 'إجراءات سريعة',
      AppStrings.loadingCommunicationDetails: 'جاري تجهيز بيانات التواصل...',
      AppStrings.years: 'سنوات',
      AppStrings.addedSuccessfully: 'تمت الإضافة',
      AppStrings.all: 'الكل',
      AppStrings.maleLawyer: 'محامي (ذكر)',
      AppStrings.femaleLawyer: 'محامية (أنثى)',
      AppStrings.defaultSort: 'الافتراضي',
      AppStrings.byRating: 'حسب التقييم',
      AppStrings.byExperience: 'حسب سنوات الخبرة',
      AppStrings.highestRated: 'الأعلى تقييماً',
      AppStrings.mostExperienced: 'الأكثر خبرة',
      AppStrings.confirmLawyersSelection: 'تأكيد اختيار {count} محامي',
      AppStrings.consultationDetails: 'تفاصيل',
      AppStrings.wallet: 'المدفوعات',
      AppStrings.deleteAccount: 'حذف الحساب',
      AppStrings.deleteAccountTitle: 'حذف الحساب نهائياً',
      AppStrings.deleteAccountSubtitle:
          'هل أنت متأكد من حذف حسابك؟ سيؤدي هذا الإجراء إلى فقدان كافة بياناتك ولا يمكن التراجع عنه.',
      AppStrings.deleteAccountAction: 'حذف حسابي',
      AppStrings.totalPayments: 'إجمالي المدفوعات',
      AppStrings.accountActive: 'الحساب نشط',
      AppStrings.completed: 'مكتمل',
      AppStrings.phone: 'رقم الهاتف',
      AppStrings.emailOrPasswordNotValid:
          'البريد الإلكتروني أو كلمة المرور غير صحيحة',
      AppStrings.cancelOrder: 'إلغاء',
      AppStrings.cancelCase: 'إلغاء',
      AppStrings.confirmCancelCase: 'هل أنت متأكد من إلغاء القضية؟',
      AppStrings.contactLegalOffice: 'تواصل مع المكتب القانوني',
      AppStrings.referenceNumber: 'رقم مرجع الاستشارة',
      AppStrings.consultationSubject: 'موضوع الاستشارة',
      AppStrings.consultationPlaceholder:
          'استشارة قانونية عامة حول الأنظمة واللوائح المعمول بها.',
      AppStrings.attachedDocuments: 'المرفقات',
      AppStrings.reviewStep: 'مراجعة',
      AppStrings.analysisStep: 'التحليل',
      AppStrings.appointmentStep: 'الموعد',
      AppStrings.executionStep: 'التنفيذ',
      AppStrings.rateConsultation: 'تقييم التجربة الاستشارية',
      AppStrings.rateConsultationDesc:
          'رأيك يساهم في تحسين جودة خدماتنا القانونية',
      AppStrings.additionalNotes: 'ملاحظات إضافية:',
      AppStrings.writeNotesHint: 'اكتب ملاحظاتك هنا (اختياري)...',
      AppStrings.financialStatement: 'البيان المالي للخدمة',
      AppStrings.consultationFee: 'رسوم الاستشارة',
      AppStrings.subtotal: 'المجموع الفرعي',
      AppStrings.specialDiscount: 'خصم خاص',
      AppStrings.totalProfessionalFees: 'إجمالي الرسوم المهنية:',
      AppStrings.licensedLawyer: 'محامي مرخص',
      AppStrings.ratingLabel: 'التقييم',
      AppStrings.aboutLawyer: 'عن المحامي',
      AppStrings.servicesAndPrices: 'الخدمات والأسعار',
      AppStrings.clientReviews: 'آراء العملاء',
      AppStrings.viewAll: 'عرض الكل',
      AppStrings.requestConsultationNow: 'طلب استشارة الآن',
      AppStrings.receptionSettings: 'إعدادات الاستقبال',
      AppStrings.acceptChatConsultations: 'استشارات نصية',
      AppStrings.acceptInstantConsultations: 'استشارات فورية',
      AppStrings.acceptServices: 'خدمات قانونية',
      AppStrings.performanceOverview: 'نظرة عامة على الأداء',
      AppStrings.activeCasesLabel: 'قضايا جارية',
      AppStrings.monthlyIncome: 'دخل الشهر',
      AppStrings.videoCall: 'مكالمة',
      AppStrings.professionalTools: 'منصة العمل',
      AppStrings.legalLibrary: 'المكتبة القانونية',
      AppStrings.myTasks: 'مهامي',
      AppStrings.reports: 'التقارير',
      AppStrings.clientsRating: 'تقييم العملاء',
      AppStrings.allAppointments: 'كل المواعيد',
      AppStrings.availableOnline: 'متاح',
      AppStrings.busy: 'مشغول',
      AppStrings.availableBalance: 'الرصيد المتاح',
      AppStrings.lastTransactions: 'آخر العمليات',
      AppStrings.withdrawalActionMsg:
          'سيتم تفعيل طلبات السحب للرصيد المتاح قريباً',
      AppStrings.pendingEarnings: 'أرباح معلقة',
      AppStrings.deductedCommission: 'العمولة المستقطعة',
      AppStrings.viewOptions: 'خيارات العرض',
      AppStrings.byDateNewest: 'حسب التاريخ (الأحدث أولاً)',
      AppStrings.byAmountHighest: 'حسب المبلغ (الأعلى أولاً)',
      AppStrings.onlyBankTransfers: 'التحويلات البنكية فقط',
      AppStrings.noTransactions: 'لا توجد عمليات',
      AppStrings.noTransactionsSubtitle:
          'لم يتم العثور على عمليات ضمن هذا التصنيف.',
      AppStrings.income: 'دخل',
      AppStrings.expenses: 'مصروفات',
      AppStrings.myDailyTasks: 'مهامي اليومية',
      AppStrings.upcomingTasks: 'المهام القادمة',
      AppStrings.totalTasks: 'مهام إجمالية',
      AppStrings.pendingAction: 'بانتظار العمل',
      AppStrings.highPriority: 'عالية',
      AppStrings.mediumPriority: 'متوسطة',
      AppStrings.lowPriority: 'منخفضة',
      AppStrings.today: 'اليوم',
      AppStrings.tomorrow: 'غداً',
      AppStrings.am: 'ص',
      AppStrings.pm: 'م',
      AppStrings.sunday: 'أحد',
      AppStrings.monday: 'اثنين',
      AppStrings.tuesday: 'ثلاثاء',
      AppStrings.wednesday: 'أربعاء',
      AppStrings.thursday: 'خميس',
      AppStrings.friday: 'جمعة',
      AppStrings.saturday: 'سبت',
      AppStrings.appNameArabic: 'حُجّة',
      AppStrings.splashSubtitle: 'استشاراتك القانونية بين يديك',
      AppStrings.joinhoggaTeam: 'انضم لفريق حُجّة',
      AppStrings.stepLabel: 'الخطوة',
      AppStrings.stepDataLabel: 'بيانات',
      AppStrings.stepAccount: 'الحساب',
      AppStrings.stepIdentity: 'الهوية',
      AppStrings.stepSpecialization: 'التخصص',
      AppStrings.stepServices: 'الخدمات',
      AppStrings.stepDocuments: 'المستندات',
      AppStrings.stepReview: 'المراجعة',
      AppStrings.stepStatus: 'الحالة',
      AppStrings.confirmAndSendRequest: 'تأكيد وإرسال الطلب',
      AppStrings.continueAction: 'المتابعة',
      AppStrings.createAccountTitle: 'إنشاء حسابك',
      AppStrings.createAccountSubtitle: 'أدخل بياناتك الأساسية للبدء في حُجّة',
      AppStrings.otpSentSubtitle: 'أدخل رمز التحقق المرسل لهاتفك',
      AppStrings.otpSentNotice: 'تم إرسال رمز التحقق إلى الهاتف المسجل',
      AppStrings.resendCodeAction: 'إعادة إرسال الرمز؟',
      AppStrings.personalDataTitle: 'البيانات الشخصية',
      AppStrings.personalDataSubtitle:
          'هذه البيانات تساعدنا في التحقق من هويتك الشخصية',
      AppStrings.fullNameIdHint: 'الاسم بالكامل (كما في الهوية)',
      AppStrings.civilIdHint: 'الرقم المدني / الهوية',
      AppStrings.requiredError: 'مطلوب',
      AppStrings.fullNameMinError: 'يرجى إدخال 3 أسماء على الأقل',
      AppStrings.professionalDataTitle: 'البيانات المهنية',
      AppStrings.professionalDataSubtitle:
          'أدخل تفاصيل ممارستك القانونية المهنية',
      AppStrings.licenseNumberHint: 'رقم القيد المهني',
      AppStrings.degreeHint: 'درجة القيد',
      AppStrings.experienceYearsHint: 'سنوات الخبرة',
      AppStrings.legalSpecsTitle: 'التخصصات القانونية',
      AppStrings.servicesAndPricesTitle: 'الخدمات والأسعار',
      AppStrings.servicesAndPricesSubtitle:
          'حدد الخدمات التي تقدمها وسعر كل خدمة بالريال العماني (ر.ع)',
      AppStrings.setPricesTitle: 'تحديد الأسعار لكل خدمة',
      AppStrings.minOneServiceError: 'يرجى اختيار خدمة واحدة على الأقل',
      AppStrings.priceHint: 'السعر',
      AppStrings.fixedType: 'ثابت',
      AppStrings.averageType: 'متوسط',
      AppStrings.officialDocsTitle: 'المستندات الرسمية',
      AppStrings.officialDocsSubtitle:
          'يرجى رفع صور واضحة للمستندات المطلوبة للتحقق من المهنية',
      AppStrings.idCardPhoto: 'صورة بطاقة الهوية',
      AppStrings.licensePhoto: 'صورة كارنيه النقابة / الرخصة',
      AppStrings.registrationCertOptional: 'شهادة قيد (اختياري)',
      AppStrings.clickToUpload: 'اضغط للرفع',
      AppStrings.reviewApplicationTitle: 'مراجعة طلبك',
      AppStrings.reviewApplicationSubtitle:
          'تأكد من صحة كافة البيانات المدخلة قبل إرسال الطلب للمراجعة',
      AppStrings.experienceLabel: 'الخبرة',
      AppStrings.specializationsLabel: 'التخصصات',
      AppStrings.applicationUnderReviewTitle: 'طلبك قيد المراجعــة',
      AppStrings.applicationUnderReviewSubtitle:
          'نحن نعمل الآن على التحقق من بياناتك ومستنداتك القانونية لضمان جودة فريق حُجّة. سنقوم بإبلاغك عبر رسالة نصية فور القبول.',
      AppStrings.legalConsultation: 'استشارة قانونية',
      AppStrings.prefsAndApp: 'التفضيلات والتطبيق',
      AppStrings.clearAll: 'مسح الكل',
      AppStrings.newLegalRequest: 'طلب قانوني جديد',
      AppStrings.sessionReminder: 'تذكير بموعد جلسة',
      AppStrings.caseUpdate: 'تحديث في قضية',
      AppStrings.newDocumentUploaded: 'مستند جديد مرفوع',
      AppStrings.consultationBooking: 'حجز موعد استشارة',
      AppStrings.since: 'منذ',
      AppStrings.minutesLabel: 'دقيقة',
      AppStrings.callMinutes: 'الدقائق المتبقية من المكالمة',
      AppStrings.callEnded: 'انتهت المكالمة',
      AppStrings.reconnectingCall: 'جارٍ إعادة الاتصال...',
      AppStrings.callDurationLabel: 'مدة المكالمة',
      AppStrings.ok: 'حسنًا',
      AppStrings.performanceReports: 'تقارير الأداء',
      AppStrings.weekly: 'أسبوعي',
      AppStrings.monthly: 'شهري',
      AppStrings.yearly: 'سنوي',
      AppStrings.annualIncome: 'الدخل السنوي',
      AppStrings.consultationIncome: 'دخل الاستشارات',
      AppStrings.recentWeeks: 'الأسابيع الأخيرة',
      AppStrings.recentMonths: 'الأشهر الأخيرة',
      AppStrings.activitySummary: 'ملخص النشاط',
      AppStrings.completedCases: 'القضايا المكتملة',
      AppStrings.clientSatisfaction: 'رضا العملاء',
      AppStrings.basedOnEvaluations: 'بناءً على {count} تقييم',
      AppStrings.noClients: 'لا يوجد عملاء',
      AppStrings.noClientsSubtitle: 'لم يقم أي عميل بالتواصل معك بعد.',
      AppStrings.searchByClientName: 'البحث باسم العميل...',
      AppStrings.activeCasesCount: '{count} قضايا نشطة',
      AppStrings.callUnderDevelopment: 'الاتصال قيد التطوير',
      AppStrings.chatUnderDevelopment: 'المحادثة قيد التطوير',
      AppStrings.inProgress: 'الجارية',
      AppStrings.noCases: 'لا توجد قضايا',
      AppStrings.noCasesSubtitle: 'لم يتم إسناد أي قضايا لك حتى الآن.',
      AppStrings.noSearchResultsSubtitle:
          'جرب البحث عن كلمة أخرى أو تغيير الفلترة.',
      AppStrings.searchByCaseIdOrTitle: 'البحث برقم القضية أو العنوان...',
      AppStrings.caseNumber: 'رقم القضية: {id}',
      AppStrings.courtLabel: 'المحكمة',
      AppStrings.primaryCourt: 'ابتدائية',
      AppStrings.caseDetails: 'تفاصيل القضية',
      AppStrings.upcoming: 'قادم',
      AppStrings.canceled: 'ملغى',
      AppStrings.scheduleTable: 'جدول المواعيد',
      AppStrings.noBookings: 'لا توجد حجوزات',
      AppStrings.noBookingsSubtitle: 'لا يوجد لديك أي حجوزات قادمة.',
      AppStrings.availableForConsultations: 'أنت متاح لاستقبال الاستشارات',
      AppStrings.currentlyPaused: 'أنت متوقف حالياً',
      AppStrings.profileVisibleNotice: 'سيظهر ملفك للعملاء',
      AppStrings.profileHiddenNotice: 'لن يتمكن العملاء من حجز مواعيد جديدة',
      AppStrings.confirmedStatus: 'مؤكد',
      AppStrings.pendingStatus: 'قيد الانتظار',
      AppStrings.cancelledStatus: 'ملغي',
      AppStrings.deliveredStatus: 'تم التوصيل',
      AppStrings.shippedStatus: 'جاري الشحن',
      AppStrings.unknownStatus: 'مجهول',
      AppStrings.cancelBooking: 'إلغاء الموعد',
      AppStrings.enterOtpSentTo: 'أدخل الرمز المرسل إلى {email}',
      AppStrings.verificationSuccess: 'تم التحقق بنجاح',
      AppStrings.resendCode: 'إعادة الإرسال',
      AppStrings.resendIn: 'إعادة الإرسال خلال {seconds} ث',
      AppStrings.specializedConsultation: 'استشارة تخصصية',
      AppStrings.contractDrafting: 'صياغة عقد',
      AppStrings.caseStudyService: 'دراسة قضية',
      AppStrings.priceWithCurrency: '{price} ر.ع',
      AppStrings.comingSoon: 'قريباً',
      AppStrings.saudi: 'السعودية',
      AppStrings.uae: 'الإمارات',
      AppStrings.oman: 'سلطنة عمان',
      AppStrings.kuwait: 'الكويت',
      AppStrings.qatar: 'قطر',
      AppStrings.bahrain: 'البحرين',
      AppStrings.egypt: 'مصر',
      AppStrings.jordan: 'الأردن',
      AppStrings.redirectingToService: 'تم توجيهك لطلب الخدمة',
      AppStrings.chatUnderDevelopmentSubtitle: 'بدء المحادثة قيد التطوير',
      AppStrings.notificationNewRequestDesc:
          'لديك طلب استشارة عقارية جديد من خالد المعمري.',
      AppStrings.notificationSessionReminderDesc:
          'تذكير: لديك جلسة في المحكمة الابتدائية غداً الساعة 9:00 ص.',
      AppStrings.notificationCaseUpdateDesc:
          'تم تحديث حالة القضية رقم #4421 إلى "بانتظار حكم".',
      AppStrings.notificationNewDocumentDesc:
          'قام العميل برفع "لائحة دعوى المعدلة" في قضية التعويض.',
      AppStrings.notificationBookingDesc:
          'تم حجز موعد استشارة جديدة يوم الثلاثاء القادم.',
      AppStrings.secondsUnit: 'ث',
      AppStrings.verifiedSuccessfully: 'تم التحقق بنجاح',
      AppStrings.addAction: 'إضافة',
      AppStrings.orderDetail: 'تفاصيل الاستشارة',
      AppStrings.accept: 'قبول',
      AppStrings.refuse: 'رفض',
      AppStrings.acceptedSuccess: 'تم قبول الاستشارة',
      AppStrings.refusedSuccess: 'تم الرفض',
      AppStrings.waitingForApproval: 'بانتظار موافقتك',
      AppStrings.clientData: 'بيانات العميل',
      AppStrings.newClient: 'عميل جديد',
      AppStrings.requestedAppointment: 'الموعد المطلوب',
      AppStrings.implementationDurationInDays: 'مدة التنفيذ (بالأيام)',
      AppStrings.offerExpiryDate: 'تاريخ انتهاء العرض',
      AppStrings.offerValue: 'قيمة العرض (ر.ع)',
      AppStrings.offerDetails: 'تفاصيل العرض...',
      AppStrings.submitOffer: 'تقديم عرض سعر',
      AppStrings.offerExpiresAt: 'العرض ينتهي في موعد محدد',
      AppStrings.offerSentSuccess: 'تم إرسال العرض بنجاح',
      AppStrings.sendOfferNow: 'إرسال العرض الآن',
      AppStrings.following: 'متابع',
      AppStrings.follow: 'متابعة',
      AppStrings.showMore: 'عرض المزيد',
      AppStrings.showLess: 'عرض أقل',
      AppStrings.omani: 'عماني',
      AppStrings.primaryDegree: 'ابتدائي',
      AppStrings.appealDegree: 'استئناف',
      AppStrings.supremeDegree: 'نقض / عليا',
      AppStrings.specCriminalShort: 'جنائي',
      AppStrings.specCivilShort: 'مدني',
      AppStrings.specPersonalStatusShort: 'أحوال شخصية',
      AppStrings.specCorporateShort: 'شركات',
      AppStrings.specOtherShort: 'أخرى',
      AppStrings.serviceLegalConsultation: 'استشارة قانونية',
      AppStrings.serviceLegalRepresentation: 'تمثيل قانوني',
      AppStrings.serviceContractWriting: 'كتابة عقود',
      AppStrings.errorFetchBookings: 'حدث خطأ في استرجاع الحجوزات',
      AppStrings.errorFetchCases: 'حدث خطأ في استرجاع القضايا',
      AppStrings.errorFetchClients: 'حدث خطأ في استرجاع العملاء',
      AppStrings.errorFetchStats: 'حدث خطأ في استرجاع الاحصائيات',
      AppStrings.errorFetchTransactions: 'حدث خطأ في استرجاع المعاملات',
      AppStrings.errorUpdateStatus: 'حدث خطأ في تحديث الحالة',
      AppStrings.errorUpdateAvailability: 'حدث خطأ في تحديث التواجد',
      AppStrings.muscat: 'مسقط',
      AppStrings.salalah: 'صلالة',
      AppStrings.sohar: 'صحار',
      AppStrings.nizwa: 'نزوى',
      AppStrings.sur: 'صور',
      AppStrings.rustaq: 'الرستاق',
      AppStrings.buraimi: 'البريمي',
      AppStrings.ibra: 'إبراء',
      AppStrings.haima: 'هيما',
      AppStrings.khasab: 'خصب',
      AppStrings.seeb: 'السيب',
      AppStrings.bawshar: 'بوشر',
      AppStrings.mutrah: 'مطرح',
      AppStrings.amerat: 'العامرات',
      AppStrings.barka: 'بركاء',
      AppStrings.ibri: 'عبري',
      AppStrings.musannah: 'المصنعة',
      AppStrings.bahla: 'بهلاء',
      AppStrings.suwaiq: 'السويق',
      AppStrings.arabic: 'العربية',
      AppStrings.english: 'English',
      AppStrings.saudiArabia: 'السعودية',
      AppStrings.areYouALawyer: 'هل أنت محامٍ؟ ',
      AppStrings.registerAsNewLawyer: 'التسجيل كمحامٍ جديد',
      AppStrings.errorRegistrationFailed: 'فشل التسجيل',
      AppStrings.errorResetPasswordFailed: 'فشل إعادة تعيين كلمة المرور',
      AppStrings.errorVerificationFailed: 'فشل التحقق',
      AppStrings.errorResendOtpFailed: 'فشل إعادة إرسال رمز التحقق',
      AppStrings.errorServer: 'خطأ في السيرفر، يرجى المحاولة لاحقاً',
      AppStrings.noInternetConnection:
          'لا يوجد اتصال بالإنترنت. يرجى التحقق من الشبكة ثم المحاولة مرة أخرى.',
      AppStrings.requestTimedOut:
          'استغرقت العملية وقتاً أطول من المتوقع. حاول مرة أخرى.',
      AppStrings.unexpectedError: 'حدث خطأ غير متوقع. حاول مرة أخرى.',
      AppStrings.operationCancelled: 'تم إلغاء العملية.',
      AppStrings.otpResentSuccessfully: 'تمت إعادة إرسال رمز التحقق بنجاح',
      AppStrings.sessionAddedSuccessfully: 'تمت إضافة الجلسة بنجاح',
      AppStrings.documentUploadedSuccessfully: 'تم رفع المستند بنجاح',
      AppStrings.withdrawalRequestSubmittedSuccessfully:
          'تم إرسال طلب السحب بنجاح',
      AppStrings.fixed: 'ثابت',
      AppStrings.goToDashboard: 'الدخول للوحة التحكم',
      AppStrings.comma: '،',
      AppStrings.searchByLawOrLegislation: 'ابحث عن نص قانوني أو تشريع...',
      AppStrings.legalCategories: 'التصنيفات القانونية',
      AppStrings.latestReferences: 'أحدث المراجع',
      AppStrings.criminalLaw: 'القانون الجنائي',
      AppStrings.commercialLaw: 'القانون التجاري',
      AppStrings.personalStatus: 'الأحوال الشخصية',
      AppStrings.laborLaw: 'قانون العمل',
      AppStrings.civilLaw: 'القانون المدني',
      AppStrings.omanLaws: 'قوانين سلطنة عمان',
      AppStrings.documentsAndFiles: 'المستندات والملفات',
      AppStrings.recentFolders: 'المجلدات الأخيرة',
      AppStrings.latestFiles: 'أحدث الملفات',
      AppStrings.folderName: 'اسم المجلد',
      AppStrings.openDocument: 'عرض',
      AppStrings.addTask: 'إضافة مهمة',
      AppStrings.taskTitle: 'عنوان المهمة',
      AppStrings.selectPriority: 'اختر الأولوية',
      AppStrings.high: 'عالية',
      AppStrings.medium: 'متوسطة',
      AppStrings.low: 'منخفضة',
      AppStrings.legalArticles: 'المقالات القانونية',
      AppStrings.views: 'مشاهدة',
      AppStrings.createdAt: 'تاريخ النشر',
      AppStrings.notifications: 'الإشعارات',
      AppStrings.dueDate: 'تاريخ الاستحقاق',
      AppStrings.uploadNewDocument: 'رفع مستند جديد',
      AppStrings.cancel: 'إلغاء',
      AppStrings.filesCount: '{count} ملف/ملفات',
      AppStrings.filter: 'تصفية',
      AppStrings.requestWithdrawal: 'طلب سحب',
      AppStrings.clients: 'العملاء',
      AppStrings.consultationsTab: 'الاستشارات',
      AppStrings.more: 'المزيد',
      AppStrings.totalBookings: 'إجمالي الحجوزات',
      AppStrings.nextAppointment: 'الموعد القادم',
      AppStrings.documents: 'المستندات',
      AppStrings.noResults: 'لا توجد نتائج',
      AppStrings.noResultsSubtitle:
          'حاول البحث عن كلمة أخرى أو تغيير نص البحث.',
      AppStrings.myCases: 'القضايا',
      AppStrings.mySubscription: 'اشتراكاتي',
      AppStrings.serviceName: 'اسم الخدمة',
      AppStrings.client: 'العميل',
      AppStrings.date: 'التاريخ',
      AppStrings.startSession: 'بدء الجلسة',
      AppStrings.confirmed: 'مؤكد',

      // NEW ADDITIONS
      AppStrings.otpSentMessage: 'تم إرسال رمز التحقق بنجاح',
      AppStrings.backToLogin: 'العودة لتسجيل الدخول',
      AppStrings.resetPassword: 'إعادة تعيين كلمة المرور',
      AppStrings.passwordChanged: 'تم تغيير كلمة المرور بنجاح',
      AppStrings.otpCode: 'رمز التحقق',
      AppStrings.otpMustBe4: 'يجب أن يكون الرمز 6 أرقام',
      AppStrings.invalidOtp: 'يرجى إدخال رمز صحيح مكون من 6 أرقام',
      AppStrings.fullName: 'الاسم بالكامل',
      AppStrings.mobileNumber: 'رقم الجوال',
      AppStrings.genderTitle: 'الجنس',
      AppStrings.gender: 'الجنس',
      AppStrings.accountTierTitle: 'نوع الحساب',
      AppStrings.profileUpdatedSuccessfully: 'تم تحديث الملف الشخصي بنجاح',
      AppStrings.logoutTitle: 'تسجيل الخروج',
      AppStrings.logoutSubtitle: 'هل أنت متأكد أنك تريد تسجيل الخروج؟',
      AppStrings.searchHint: 'ابحث هنا...',
      AppStrings.hello: 'أهلاً',
      AppStrings.changeLanguage: 'تغيير اللغة',
      AppStrings.support: 'الدعم الفني',
      AppStrings.back: 'رجوع',
      AppStrings.next: 'التالي',
      AppStrings.retry: 'إعادة المحاولة',
      AppStrings.weAreHereToHelp: 'نحن هنا لمساعدتك',
      AppStrings.supportTeamReady:
          'فريق الدعم الفني لدينا جاهز للرد على استفساراتكم على مدار الساعة.',
      AppStrings.phoneCall: 'اتصال هاتفي',
      AppStrings.workingHours: 'ساعات العمل',
      AppStrings.error: 'خطأ',
      AppStrings.loginFailed: 'فشل تسجيل الدخول',
      AppStrings.errorUnrecognized: 'خطأ غير معروف',
      AppStrings.loginWithGoogle: 'تسجيل الدخول باستخدام جوجل',
      AppStrings.registerWithGoogle: 'إنشاء حساب باستخدام جوجل',
      AppStrings.fingerprintLogin: 'الدخول بالبصمة',
      AppStrings.transactionHistory: 'سجل المعاملات',
      AppStrings.rateAppStore: 'تقييم التطبيق',
      AppStrings.dataPrivacy: 'خصوصية البيانات',
      AppStrings.confirm: 'تأكيد',
      AppStrings.currency: 'ر.ع',

      AppStrings.publicOpportunities: 'الفرص والطلبات العامة',
      AppStrings.myServices: 'خدماتي',
      AppStrings.mySpecializations: 'تخصصاتي',
      AppStrings.addService: 'إضافة خدمة',
      AppStrings.editService: 'تعديل الخدمة',
      AppStrings.serviceType: 'نوع الخدمة',
      AppStrings.price: 'السعر',
      AppStrings.serviceDescription: 'وصف الخدمة',
      AppStrings.submitProposal: 'تقديم عرض',
      AppStrings.proposalDescription: 'تفاصيل العرض (رسالة العرض)',
      AppStrings.proposalPrice: 'سعر العرض',
      AppStrings.deliveryTime: 'مدة التنفيذ',
      AppStrings.days: 'أيام',
      AppStrings.noOpportunities: 'لا توجد طلبات عامة متاحة في الوقت الحالي.',
      AppStrings.noServices: 'لم تقم بإضافة أي خدمات حتى الآن.',
      AppStrings.serviceAddedSuccess: 'تم إضافة الخدمة بنجاح',
      AppStrings.proposalSubmittedSuccess: 'تم تقديم العرض بنجاح',
      AppStrings.lawyerDashboardTitle: 'لوحة تحكم المحامي',
      AppStrings.myProposals: 'عروضي',
      AppStrings.noProposals: 'لا يوجد عروض',
      AppStrings.noProposalsSubtitle: 'لم تقم بتقديم أي عروض حتى الآن.',
      AppStrings.caseNumberLabel: 'رقم القضية',
      AppStrings.yourOffer: 'عرضك',
      AppStrings.currencyRial: 'ر.ع',
      AppStrings.priceRange: 'نطاق السعر',
      AppStrings.deleteProposal: 'حذف العرض',
      AppStrings.confirmDeleteProposal:
          'هل أنت متأكد من رغبتك في حذف هذا العرض؟',
      AppStrings.cancelProposal: 'إلغاء العرض',
      AppStrings.confirmCancelProposal:
          'هل أنت متأكد من رغبتك في إلغاء هذا العرض وسحبه؟',
      AppStrings.editProposal: 'تعديل العرض',
      AppStrings.optionalDocuments: 'المستندات (اختياري)',
      AppStrings.addFile: 'إضافة ملف',
      AppStrings.accepted: 'مقبول',
      AppStrings.acceptedProposal: 'تم قبول العرض',
      AppStrings.rejected: 'مرفوض',
      AppStrings.pending: 'قيد الانتظار',
      AppStrings.pendingReview: 'قيد المراجعة',
      AppStrings.refresh: 'تحديث',
      AppStrings.willNotifyYou: 'سنقوم بإخطارك فور الموافقة على حسابك.',
      AppStrings.pressBackAgainToExit: 'اضغط مرة أخرى للخروج من التطبيق',
      AppStrings.welcomeTo: 'مرحباً بك في تطبيق',
      AppStrings.chooseAccountType: 'اختر نوع الحساب للبدء',
      AppStrings.userRoleDescription:
          'للبحث عن محامين والحصول على استشارات قانونية',
      AppStrings.lawyerRoleDescription: 'لإدارة قضاياك والوصول إلى عملاء جدد',
      AppStrings.operationSuccess: 'تمت العملية بنجاح',
      AppStrings.proposalsCountLabel: '{count} عروض',
      AppStrings.pleaseSelectSessionDate: 'الرجاء اختيار تاريخ الجلسة',
      AppStrings.sessionTitleHint: 'عنوان التحديث/الجلسة',
      AppStrings.sessionDateLabel: 'تاريخ الجلسة',
      AppStrings.sessionDetailsHint: 'تفاصيل الجلسة',
      AppStrings.pleaseSelectFile: 'الرجاء اختيار ملف لرفعه',
      AppStrings.documentTitleHint: 'عنوان المستند',
      AppStrings.clickToSelectFile: 'انقر لاختيار ملف (PDF, PNG, JPG)',
      AppStrings.byClient: 'بواسطة العميل',
      AppStrings.byLawyer: 'بواسطة المحامي',
      AppStrings.active: 'نشط',
      AppStrings.notActive: 'غير نشط',
      AppStrings.paused: 'متوقف مؤقتاً',
      AppStrings.inCourts: 'في المحاكم',
      AppStrings.contractWriting: 'كتابة عقود',
      AppStrings.remoteConsultation: 'استشارة عن بعد',
      AppStrings.manageSchedule: 'إدارة الجدول',
      AppStrings.myClients: 'العملاء',
      AppStrings.pleaseCompleteAllFields: 'الرجاء إكمال كافة الحقول',
      AppStrings.selectionType: 'طريقة الاختيار',
      AppStrings.paymentMethod: 'طريقة الدفع',
      AppStrings.paymentStatus: 'حالة الدفع',
      AppStrings.discount: 'الخصم',
      AppStrings.receivedProposals: 'العروض المقدمة',
      AppStrings.confirmAcceptProposal:
          'هل أنت متأكد من قبول العرض المقدم من {name}؟',
      AppStrings.enterDocumentTitles: 'أدخل عنوان كل مستند',
      AppStrings.documentTitle: 'عنوان المستند',
      AppStrings.uploadDocuments: 'رفع المستندات',
      AppStrings.pleaseEnterDocumentTitles: 'يرجى إدخال عنوان لكل مستند',
      AppStrings.pleaseSelectExecutionDate: 'يرجى اختيار تاريخ ووقت التنفيذ',
      AppStrings.pleaseSelectAtLeastOneLawyer:
          'يرجى اختيار محامٍ واحد على الأقل',
      AppStrings.executionDateTime: 'تاريخ ووقت التنفيذ',
      AppStrings.chooseExecutionDateTime: 'اختر تاريخ ووقت التنفيذ',
      AppStrings.couponApplied: 'تم تطبيق الكوبون: {code}',
      AppStrings.discountLabel: 'الخصم:',
      AppStrings.submitting: 'جاري الإرسال...',
      AppStrings.budgetRange: 'نطاق الميزانية',
      AppStrings.minPrice: 'الحد الأدنى',
      AppStrings.maxPrice: 'الحد الأقصى',
      AppStrings.pleaseEnterMaxPrice: 'يرجى تحديد الحد الأقصى للسعر',
      AppStrings.newRequest: 'طلب جديد',

      // About Us Arabic
      AppStrings.aboutUsDescription:
          'حُجّة هي منصة قانونية متكاملة تربط الموكلين بنخبة من المحامين المرخصين بكل سهولة وأمان، لضمان وصول الجميع إلى الدعم القانوني الاحترافي.',
      AppStrings.ourGoals: 'أهدافنا',
      AppStrings.ourGoalsContent:
          'نهدف إلى تسهيل الوصول إلى العدالة وتقديم الدعم القانوني للجميع من خلال توظيف أحدث التقنيات الرقمية في المجال القانوني.',
      AppStrings.linkBetweenUserAndProvider:
          'نحن حلقة الوصل الموثوقة بينك وبين الخبراء القانونيين لضمان حقوقك.',
      AppStrings.aboutUsContent:
          'فريقنا مكرس لتقديم استشارات قانونية عالية الجودة تتسم بالشفافية والاحترافية.',
      AppStrings.whyZoneApp: 'لماذا تطبيق حُجّة؟',
      AppStrings.rateApp: 'تقييم حُجّة',

      // Privacy Policy Arabic
      AppStrings.privacyIntroTitle: 'مقدمة',
      AppStrings.privacyIntroContent:
          'نحن في حُجّة نولي أهمية قصوى لخصوصيتك وحماية بياناتك وفقاً لأعلى المعايير الأمنية الدولية.',
      AppStrings.dataWeCollectTitle: 'جمع البيانات',
      AppStrings.dataWeCollectContent:
          'نقوم بجمع البيانات الضرورية فقط لتقديم وتحسين خدماتنا القانونية لك.',
      AppStrings.paymentDataNotice:
          'تتم معالجة بيانات الدفع بشكل آمن تماماً عبر شركاء دفع مرخصين وموثوقين.',
      AppStrings.howWeUseDataTitle: 'استخدام البيانات',
      AppStrings.howWeUseDataContent:
          'تستخدم بياناتك لربطك بالمحامين المناسبين وإدارة طلباتك وقضاياك بكفاءة.',
      AppStrings.dataSharingTitle: 'مشاركة البيانات',
      AppStrings.dataSharingContent:
          'لا نشارك بياناتك إلا مع المحامين الذين تختار التعامل معهم وبناءً على رغبتك.',
      AppStrings.dataProtectionTitle: 'حماية البيانات',
      AppStrings.dataProtectionContent:
          'نستخدم تقنيات تشفير متقدمة لضمان حماية معلوماتك الشخصية من أي وصول غير مصرح به.',
      AppStrings.userRightsTitle: 'حقوقك',
      AppStrings.userRightsContent:
          'لديك الحق في الوصول إلى بياناتك أو تصحيحها أو طلب حذفها في أي وقت.',
      AppStrings.policyUpdatesTitle: 'تحديثات السياسة',
      AppStrings.policyUpdatesContent:
          'قد نقوم بتحديث هذه السياسة دورياً وسنقوم بإشعارك بأي تغييرات جوهرية تطرأ عليها.',

      // General Provisions Arabic
      AppStrings.generalProvisions:
          'توضح هذه الإرشادات طريقة استخدام منصة حُجّة وخدماتها بشكل صحيح وآمن.',

      // Lawyer Proposals & Services Arabic
      AppStrings.services: 'الخدمات',
      AppStrings.clientRequests: 'طلبات العملاء',
      AppStrings.availableServices: 'الخدمات المتاحة',
      AppStrings.settings: 'الإعدادات',
      AppStrings.submitPriceOffer: 'تقديم عرض',
      AppStrings.acceptTextConsultations: 'استشارات نصية',
      AppStrings.acceptScheduledConsultations: 'استشارات مجدولة',
      AppStrings.manageConsultationPrices: 'إدارة أسعار الاستشارات',
      AppStrings.consultationType: 'نوع الاستشارة',
      AppStrings.addPriceOption: 'إضافة مدة وسعر',
      AppStrings.savePrices: 'حفظ الأسعار',
      AppStrings.durationMinutes: 'المدة بالدقائق',
      AppStrings.priceAmount: 'قيمة السعر',
      AppStrings.totalBalance: 'إجمالي الرصيد',
      AppStrings.totalSent: 'إجمالي المسحوبات',
      AppStrings.totalPurchases: 'إجمالي الطلبات',
      AppStrings.pendingPurchases: 'طلبات قيد التنفيذ',
      AppStrings.completedPurchases: 'طلبات مكتملة',
      AppStrings.totalIncome: 'إجمالي الدخل',
      AppStrings.serviceDetails: 'تفاصيل الخدمة',
      AppStrings.activeLabel: 'نشط',
      AppStrings.inactiveLabel: 'غير نشط',
      AppStrings.confirmDelete: 'هل أنت متأكد من حذف هذه الخدمة؟',
      AppStrings.applicationSentSuccess: 'تم إرسال طلبك بنجاح',
      AppStrings.goToLawyerDashboard: 'الانتقال إلى لوحة المحامي',
      AppStrings.chooseAccountTypeSubtitle: 'حدد نوع الحساب المناسب لك.',
      AppStrings.applicationUnderReviewNotice:
          'تم استكمال التسجيل، وحسابك الآن بانتظار موافقة الإدارة قبل التفعيل الكامل.',
      AppStrings.personalPhoto: 'الصورة الشخصية',
      AppStrings.photoUploadDesc: 'ارفع صورة واضحة بصيغة PNG أو JPG أو WEBP.',
      AppStrings.nameRequired: 'الاسم مطلوب',
      AppStrings.validNameRequired: 'يرجى إدخال اسم واضح.',
      AppStrings.civilId: 'الرقم المدني',
      AppStrings.expRequired: 'سنوات الخبرة مطلوبة',
      AppStrings.validExpRequired: 'يرجى إدخال رقم صحيح',
      AppStrings.alreadyHaveAccountLogin: 'لديك حساب بالفعل؟ تسجيل الدخول',
      AppStrings.additionalDataTitle: 'أكمل البيانات الإضافية',
      AppStrings.additionalDataSubtitle:
          'هذه الحقول يحددها السيرفر حسب نوع الحساب الذي اخترته. الحقول الموسومة بنجمة مطلوبة.',
      AppStrings.submitAndCompleteRegistration: 'إرسال واستكمال التسجيل',
      AppStrings.allowedFilesDesc:
          'الملفات المسموح بها: PDF, JPG, PNG, WEBP حتى 5 ميجابايت.',
      AppStrings.uploadFile: 'رفع {name}',
      AppStrings.chooseFileHint: 'اختر الملف المناسب لهذا الحقل',
      AppStrings.enterNumberHint: 'أدخل رقم {name}',
      AppStrings.enterTextHint: 'أدخل {name}',
      AppStrings.required: 'مطلوب',
      AppStrings.optional: 'اختياري',
      AppStrings.chooseSpecsSubtitle: 'يمكنك اختيار أكثر من تخصص واحد.',
      AppStrings.specsSelectedCount: 'تم اختيار {count} تخصص',
      AppStrings.verificationCodeTitle: 'التحقق من رقم الهاتف',
      AppStrings.verificationCodeHint: 'رمز التحقق',
      AppStrings.verifyCodeButton: 'تأكيد الرمز',
      AppStrings.enterVerificationCode:
          'أدخل رمز التحقق المرسل إلى الرقم {phone}',
      AppStrings.browseOpportunities: 'تصفح الفرص المتاحة',
      AppStrings.noServicesSubtitle:
          'لم تقم بإضافة أي خدمات قانونية بعد. ابدأ الآن بإضافة خدمتك الأولى.',
      AppStrings.addNewService: 'أضف خدمة جديدة',
      AppStrings.legalRepresentation: 'تمثيل قانوني',
      AppStrings.companyFormation: 'تأسيس شركات',
      AppStrings.pleaseSelectServiceType: 'يرجى اختيار نوع الخدمة',
      AppStrings.markAllAsRead: 'تحديد الكل كمقروء',
      AppStrings.noNotifications: 'لا توجد إشعارات',
      AppStrings.noNotificationsSubtitle:
          'سنقوم بإخطارك عند وجود تحديثات جديدة.',
      AppStrings.callNoLongerAvailable: 'هذه المكالمة لم تعد متاحة.',
      AppStrings.minutesAgo: 'منذ {count} دقيقة',
      AppStrings.hoursAgo: 'منذ {count} ساعة',
      AppStrings.daysAgo: 'منذ {count} يوم',
      AppStrings.draftRestored: 'تم استعادة المسودة السابقة بنجاح',
      AppStrings.pleaseSelectAccountType: 'الرجاء اختيار نوع الحساب',
      AppStrings.pleaseSelectAtLeastOneSpec:
          'الرجاء اختيار تخصص واحد على الأقل',
      AppStrings.pleaseUploadPersonalPhoto: 'الرجاء رفع الصورة الشخصية',
      AppStrings.pleaseUploadField: 'الرجاء رفع {field}',
      AppStrings.pleaseEnterField: 'الرجاء إدخال {field}',
      AppStrings.unsupportedFileFormat: 'صيغة الملف غير مدعومة',
      AppStrings.fileSizeTooLarge: 'حجم الملف كبير جداً',
      AppStrings.stepXofY: 'الخطوة {current} من {total}',
      AppStrings.verificationCodeSent: 'تم إرسال رمز التحقق بنجاح',
      AppStrings.phoneVerifiedSuccess: 'تم التحقق من رقم الهاتف بنجاح',
      AppStrings.selectAccountTypeFirst: 'الرجاء اختيار نوع الحساب أولاً',
      AppStrings.basicInfoSaved: 'تم حفظ البيانات الأساسية بنجاح',
      AppStrings.phoneAlreadyRegistered: 'رقم الهاتف هذا مسجل مسبقاً',
      AppStrings.emailAlreadyRegistered: 'البريد الإلكتروني هذا مسجل مسبقاً',
      AppStrings.noOpportunitiesSubtitle:
          'لا توجد طلبات عملاء متاحة في تخصصاتك. سنقوم بإخطارك عند ظهور فرص جديدة.',
      AppStrings.emptyServicesHistory: 'سجل الخدمات المتاحة فارغ حالياً.',
      AppStrings.noActiveOrdersSubtitle:
          'ليس لديك طلبات نشطة حالياً. يمكنك تصفح الخدمات وطلب استشارة جديدة.',
      AppStrings.emptyOrderHistory: 'سجل الطلبات فارغ حالياً.',
      AppStrings.maintenanceDefaultMessage:
          'نحن نقوم ببعض التحسينات. سنعود قريباً.',
      AppStrings.updateDefaultMessage: 'يرجى تحديث التطبيق للاستمرار.',
      AppStrings.processingAndVerifying: 'جاري المعالجة والتحقق...',
      AppStrings.fileVerifiedSuccess: 'تم التحقق من الملف بنجاح',
      AppStrings.welcome: 'أهلاً بك',
      AppStrings.addCaseUpdate: 'إضافة تحديث للقضية',
      AppStrings.clientDetails: 'تفاصيل العميل',
      AppStrings.sessionsHistory: 'سجل الجلسات',
      AppStrings.noDataFound: 'لا توجد بيانات',

      AppStrings.changePassword: 'تغيير كلمة المرور',
      AppStrings.other: 'أخرى',

      AppStrings.selectDate: 'اختر التاريخ',

      AppStrings.priceExceedsBudget: 'لا يمكن أن يتجاوز السعر ميزانية العميل',
      AppStrings.deleteTask: 'حذف المهمة',
      AppStrings.confirmDeleteTask: 'هل أنت متأكد من حذف هذه المهمة؟',
      AppStrings.noTasks: 'لا توجد مهام',
      AppStrings.noTasksSubtitle: 'لم تقم بإضافة أي مهام بعد',
      AppStrings.documentName: 'اسم المستند',
      AppStrings.upload: 'رفع',
      AppStrings.noFileSelected: 'لم يتم اختيار ملف',
      AppStrings.deleteDocument: 'حذف المستند',
      AppStrings.confirmDeleteDocument: 'هل أنت متأكد من حذف هذا المستند؟',
      AppStrings.pendingBalance: 'الرصيد المعلق',
      AppStrings.amount: 'المبلغ',
      AppStrings.accountName: 'اسم صاحب الحساب',
      AppStrings.bankName: 'اسم البنك',
      AppStrings.iban: 'رقم الآيبان (IBAN)',
      AppStrings.sendRequest: 'إرسال الطلب',
      AppStrings.viewDetails: 'عرض التفاصيل',
      AppStrings.apply: 'تطبيق',

      AppStrings.confirmAccept: 'تأكيد القبول',
      AppStrings.confirmRefuse: 'تأكيد الرفض',
      AppStrings.areYouSure: 'هل أنت متأكد من رغبتك في تنفيذ هذا الإجراء؟',
      AppStrings.rateLawyer: 'تقييم المحامي',
      AppStrings.rateLawyerTitle: 'قيّم {name}',
      AppStrings.rateLawyerSubtitle:
          'شارك تجربتك لمساعدة العملاء الآخرين على الاختيار بثقة.',
      AppStrings.ratingComment: 'التعليق',
      AppStrings.ratingCommentHint: 'اكتب رأيك هنا...',
      AppStrings.submitRating: 'إرسال التقييم',
      AppStrings.ratingSentSuccessfully: 'تم إرسال التقييم بنجاح',
      AppStrings.description: 'الوصف',
      AppStrings.subscriptionPackages: 'باقات الاشتراك',
      AppStrings.currentSubscription: 'الاشتراك الحالي',
      AppStrings.consultationLeft: 'استشارة متبقية',
      AppStrings.choosePackage: 'اختر الباقة المناسبة',
      AppStrings.notVisibleToClients: 'الملف الشخصي غير ظاهر للعملاء',
      AppStrings.activateSubscriptionToStart:
          'يرجى تفعيل الاشتراك للبدء في استقبال الطلبات',
      AppStrings.subscriptionExpiringSoon: 'الاشتراك ينتهي قريباً',
      AppStrings.renew: 'تجديد',
      AppStrings.visibleToClients: 'الملف الشخصي ظاهر للعملاء',
      AppStrings.manage: 'إدارة',
      AppStrings.defaultFreePackage: 'الباقة المجانية الافتراضية',
      AppStrings.remainingDays: 'الأيام المتبقية',
      AppStrings.noExpiryDate: 'بدون تاريخ انتهاء',
      AppStrings.packages: 'الباقات',
      AppStrings.activateNow: 'تفعيل',
      AppStrings.upgradePackage: 'ترقية الباقة',
      AppStrings.subscriptionSuccess: 'تم الاشتراك في الباقة بنجاح',
      AppStrings.edit: 'تعديل',
      AppStrings.delete: 'حذف',
      AppStrings.servicesAndProducts: 'الخدمات والمنتجات',
      AppStrings.quantity: 'الكمية',
      AppStrings.paymentDetails: 'تفاصيل الدفع',
      AppStrings.paidAmount: 'المبلغ المدفوع',
      AppStrings.transactionNumber: 'رقم العملية',
      AppStrings.invoiceNumber: 'رقم الفاتورة',
      AppStrings.chats: 'المحادثات',
      AppStrings.noChats: 'لا توجد محادثات',
      AppStrings.noLawyerChatsSubtitle: 'ستظهر محادثاتك مع المحامين هنا',
      AppStrings.noClientChatsSubtitle: 'ستظهر محادثاتك مع العملاء هنا',
      AppStrings.startConversation: 'ابدأ المحادثة',
      AppStrings.calling: 'جاري الاتصال...',
      AppStrings.waitingForLawyer: 'في انتظار انضمام المحامي...',
      AppStrings.waitingForClient: 'في انتظار انضمام العميل...',
      AppStrings.connected: 'متصل',
      AppStrings.switchCamera: 'تبديل الكاميرا',
      AppStrings.speaker: 'مكبر',
      AppStrings.mute: 'كتم',
      AppStrings.video: 'فيديو',
      AppStrings.end: 'إنهاء',
      AppStrings.failedToSendMessage: 'فشل إرسال الرسالة',
      AppStrings.failedToSendFile: 'فشل إرسال الملف',
      AppStrings.voiceCall: 'مكالمة',
      AppStrings.startConversationWithLawyer: 'ابدأ المحادثة مع المحامي',
      AppStrings.startConversationWithClient: 'ابدأ المحادثة مع العميل',
      AppStrings.typeYourMessage: 'اكتب رسالتك...',
      AppStrings.pdfFile: 'ملف PDF',
      AppStrings.attachment: 'مرفق',
      AppStrings.upcomingSessions: 'الجلسات القادمة',
      AppStrings.previousSessions: 'الجلسات السابقة',
      AppStrings.completePaymentNow: 'إتمام الدفع الآن',
      AppStrings.paymentScreenTitle: 'إتمام عملية الدفع',
      AppStrings.paymentFailedDialogTitle: 'لم تكتمل عملية الدفع',
      AppStrings.paymentFailedDialogMessage:
          'حدثت مشكلة أثناء إتمام الدفع. يمكنك المحاولة مرة أخرى أو العودة للرئيسية.',
      AppStrings.cancelPaymentDialogTitle: 'هل أنت متأكد من إلغاء الدفع؟',
      AppStrings.cancelPaymentDialogMessage:
          'سيتم إيقاف عملية الدفع الحالية والعودة إلى الصفحة الرئيسية.',
      AppStrings.continuePayment: 'أكمل الدفع',
      AppStrings.paymentLinkUnavailable:
          'رابط الدفع غير متاح حالياً. حاول مرة أخرى.',
      AppStrings.makeVideoCall: 'إجراء مكالمة',
      AppStrings.makeVoiceCall: 'إجراء مكالمة',
      AppStrings.enterChat: 'دخول المحادثة',
      AppStrings.chat: 'محادثة',
      AppStrings.paidStatus: 'تم الدفع',
      AppStrings.imagePreview: 'صورة',
      AppStrings.audioPreview: 'ملف صوتي',
      AppStrings.referralCode: 'كود الإحالة',
      AppStrings.referralCodeOptional: 'كود الإحالة (اختياري)',
      AppStrings.enterReferralCodeHint: 'أدخل كود الإحالة إن وجد',
      AppStrings.referralProgram: 'برنامج الإحالات',
      AppStrings.referralHistory: 'سجل الإحالات',
      AppStrings.referralCampaignTitle: 'مكافأة استقطاب العملاء والمحامين',
      AppStrings.referralCampaignSubtitle:
          'شارك كود الإحالة الخاص بك واحصل على {amount} {currency} لكل تسجيل ناجح',
      AppStrings.copyCode: 'نسخ الكود',
      AppStrings.codeCopiedSuccess: 'تم نسخ كود الإحالة بنجاح',
      AppStrings.totalReferredUsers: 'إجمالي المستخدمين المستقطبين',
      AppStrings.referralBonus: 'بونص الإحالة',
      AppStrings.referralBonusFromReferrals: 'أرباح الإحالات',
      AppStrings.rewarded: 'تمت المكافأة',
      AppStrings.notRewarded: 'لم تتم المكافأة',
      AppStrings.invalidReferralCode: 'كود الإحالة غير صالح',
      AppStrings.noReferralsYet: 'لا توجد إحالات حتى الآن',
      AppStrings.noReferralsYetSubtitle:
          'شارك كودك مع الآخرين لتحصل على مكافآت مالية',
      AppStrings.shareCodePrompt:
          'شارك هذا الكود مع العملاء الجدد ليتم تسجيلك كمستفيد وتحصل على مكافأة مالية.',
      AppStrings.referredUser: 'المستخدم المسجل',
      AppStrings.registrationDate: 'تاريخ التسجيل',
      AppStrings.rewardAmount: 'قيمة المكافأة',
      AppStrings.details: 'التفاصيل',
      AppStrings.waitingForProposals: 'بانتظار عروض المحامين',
      AppStrings.waitingForProposalsSubtitle:
          'طلبك معروض حالياً على المحامين المعتمدين، وستظهر عروض الأسعار هنا فور تقديمها.',
      AppStrings.acceptRequestTitle: 'قبول الطلب وتحديد السعر',
      AppStrings.requestAcceptedSuccessfully:
          'تم قبول الطلب وإرسال العرض بنجاح',
    },
  };

  String translate(String key, {Map<String, String>? namedArgs}) {
    final raw = _localizedValues[locale.languageCode]?[key];
    String translation;
    if (raw != null) {
      translation = raw;
    } else {
      // Fallback: convert camelCase key to readable words
      // e.g. "cancelOrder" → "Cancel Order"
      translation = key
          .replaceAllMapped(RegExp(r'([A-Z])'), (m) => ' ${m.group(0)}')
          .trim();
      if (translation.isNotEmpty) {
        translation = translation[0].toUpperCase() + translation.substring(1);
      }
    }
    if (namedArgs != null) {
      namedArgs.forEach((k, v) {
        translation = translation.replaceAll('{$k}', v);
      });
    }
    return translation;
  }
}

class _AppLocalizationsDelegate
    extends LocalizationsDelegate<AppLocalizations> {
  const _AppLocalizationsDelegate();

  @override
  bool isSupported(Locale locale) {
    return ['en', 'ar'].contains(locale.languageCode);
  }

  @override
  Future<AppLocalizations> load(Locale locale) async {
    return AppLocalizations(locale);
  }

  @override
  bool shouldReload(LocalizationsDelegate<AppLocalizations> old) {
    return false;
  }
}

extension StringExtension on String {
  String tr(BuildContext context, {Map<String, String>? namedArgs}) {
    return AppLocalizations.of(
          context,
        )?.translate(this, namedArgs: namedArgs) ??
        this;
  }
}
